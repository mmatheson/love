...
static int inet_global_getnameinfo(lua_State *L)
{
  ...
  for (i = 1, iter = resolved; iter; i++, iter = iter->ai_next)
  {
    getnameinfo(iter->ai_addr, (socklen_t) iter->ai_addrlen, hbuf,
                host ? (socklen_t) sizeof(hbuf) : 0, sbuf, serv ? (socklen_t) sizeof(sbuf) : 0, 0);
    if (host)
    {
      lua_pushnumber(L, i);
...
int inet_meth_getpeername(lua_State *L, p_socket ps, int family)
{
  ...
  err = getnameinfo((struct sockaddr *) &peer, peer_len, name, INET6_ADDRSTRLEN, port, sizeof(port),
...
int inet_meth_getsockname(lua_State *L, p_socket ps, int family)
{
  ...
  err = getnameinfo((struct sockaddr *) &peer, peer_len, name, INET6_ADDRSTRLEN, port, 6,
                    NI_NUMERICHOST | NI_NUMERICSERV);
  if (err)
  {
    lua_pushnil(L);
...
const char *inet_trydisconnect(p_socket ps, int family, p_timeout tm)
{
  switch (family)
  {
    ...
    case AF_INET6:
    {
      ...
      return socket_strerror(socket_connect(ps, (SA *) &sin6, sizeof(sin6), tm));
    }
  }
  return NULL;
}
...
\*-------------------------------------------------------------------------*/
const char *inet_tryconnect(p_socket ps, int *family, const char *address, const char *serv,
                            p_timeout tm, struct addrinfo *connecthints)
{
  struct addrinfo *iterator = NULL, *resolved = NULL;
  const char *err = NULL;
  int current_family = *family;
  /* try resolving */
  err = socket_gaistrerror(getaddrinfo(address, serv, connecthints, &resolved));
  if (err != NULL)
  {
    if (resolved)
      freeaddrinfo(resolved);
  ...
  for (iterator = resolved; iterator; iterator = iterator->ai_next)
  {
    ...
    {
      socket_destroy(ps);
      err = inet_trycreate(ps, iterator->ai_family, iterator->ai_socktype, iterator->ai_protocol);
      if (err)
	continue;
    ...
    err = socket_strerror(
        socket_connect(ps, (SA *) iterator->ai_addr, (socklen_t) iterator->ai_addrlen, tm));
...
\*-------------------------------------------------------------------------*/
const char *inet_trybind(p_socket ps, int *family, const char *address, const char *serv,
                         struct addrinfo *bindhints)
{
  ...
  for (iterator = resolved; iterator; iterator = iterator->ai_next)
  {
    ...
    err = socket_strerror(
        socket_bind(ps, (SA *) iterator->ai_addr, (socklen_t) iterator->ai_addrlen));
