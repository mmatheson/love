...
static DXGIFormat getDXGIFormat(const DDSPixelFormat &ddpf)
{
  ...
  else if (ddpf.flags & DDPF_LUMINANCE)
  {
	...
	return DXGI_FORMAT_R8G8_UNORM;  // Some DDS writers assume the bitcount should be 8 instead
	                                // of 16
    }
...
bool isDDS(const void *data, size_t dataSize)
{
  ...
  if ((header->format.flags & DDPF_FOURCC) &&
      (header->format.fourCC == MakeFourCC('D', 'X', '1', '0')))
  {
...
DXGIFormat Parser::getFormat() const { return format; }
...
size_t Parser::getMipmapCount() const { return texData.size(); }
...
bool Parser::parseTexData(const uint8_t *data, size_t dataSize, DXGIFormat fmt, int w, int h,
                          int nb_mips)
{
  size_t offset = 0;
