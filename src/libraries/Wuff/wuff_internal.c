...
size_t wuff_calculate_samples(size_t target_size, wuff_uint8 sample_size, wuff_uint8 *head,
                              wuff_uint8 *tail)
{
  size_t samples = 0;
...
wuff_sint32 wuff_init_stream(struct wuff_handle *handle)
{
  ...
  WUFF_STATUS_BAIL()
  else if (buffer_size < WUFF_STREAM_MIN_SIZE) return WUFF_STREAM_NOT_RIFF;
  ...
  if (chunk.id != WUFF_FORMAT_CHUNK_ID)
  {
    ...
    wuff_status =
        wuff_search_chunk(handle, &chunk, &search_offset, WUFF_FORMAT_CHUNK_ID, WUFF_DATA_CHUNK_ID);
    ...
    WUFF_STATUS_BAIL()
    /* EOF bail. */
    else if (buffer_size < WUFF_HEADER_MIN_SIZE) return WUFF_STREAM_INVALID;
  }
  ...
  handle->stream.header.block_size =
      handle->stream.header.channels * handle->stream.header.bytes_per_sample;
  ...
  handle->stream.length = handle->stream.data.size / handle->stream.header.channels /
                          handle->stream.header.bytes_per_sample;
  handle->stream.position = 0;

  return WUFF_SUCCESS;
}

wuff_sint32 wuff_search_chunk(struct wuff_handle *handle, struct wuff_chunk_header *chunk,
                              wuff_uint64 *offset, wuff_uint32 id, wuff_uint32 stop_id)
{
  wuff_uint8 buffer[8];
  wuff_uint64 search_offset;
  size_t buffer_size;
...
wuff_sint32 wuff_buffer_clear(struct wuff_handle *handle)
{
  ...
  if (position < handle->stream.data.offset ||
      position > handle->stream.data.offset + handle->stream.data.size)
...
wuff_sint32 wuff_buffer_fill(struct wuff_handle *handle)
{
  ...
  wuff_status = handle->callback->read(handle->userdata, handle->buffer.data + bytes_in_buffer,
                                       &bytes_to_read);
  WUFF_STATUS_BAIL()
