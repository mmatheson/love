...
wuff_sint32 wuff_open(struct wuff_handle **handle_pointer, struct wuff_callback *callback,
                      void *userdata)
{
...
wuff_sint32 wuff_read(struct wuff_handle *handle, wuff_uint8 *out_buffer, size_t *out_size)
{
  ...
  else
  {
    ...
    *out_size = num_samples * sample_size + head + tail;
    ...
    wuff_status = wuff_buffer_release(
        handle, head_offset + head == sample_size ? num_samples + 1 : num_samples);
    WUFF_STATUS_BAIL()
  }
