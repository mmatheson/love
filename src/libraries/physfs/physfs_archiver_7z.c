...
static int szipLoadEntries(SZIPinfo *info)
{
  ...
  if (__PHYSFS_DirTreeInit(&info->tree, sizeof(SZIPentry), 1, 0))
  {
    ...
    PHYSFS_uint32 i;
    for (i = 0; i < count; i++) BAIL_IF_ERRPASS(!szipLoadEntry(info, i), 0);
    retval = 1;
  } /* if */
...
static void *SZIP_openArchive(PHYSFS_Io *io, const char *name, int forWriting, int *claimed)
{
...
static PHYSFS_Io *SZIP_openRead(void *opaque, const char *path)
{
  ...
  rc = SzArEx_Extract(&info->db, &stream.lookStream.s, entry->dbidx, &blockIndex, &outBuffer,
                      &outBufferSize, &offset, &outSizeProcessed, alloc, alloc);
...
const PHYSFS_Archiver __PHYSFS_Archiver_7Z = {
    ...
    {
        "7Z", "7zip archives", "Ryan C. Gordon <icculus@icculus.org>",
