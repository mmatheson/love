...
static inline HANDLE winFindFirstFileW(const WCHAR *path, LPWIN32_FIND_DATAW d)
{
  ...
  return FindFirstFileExW(path, FindExInfoStandard, d, FindExSearchNameMatch, NULL, 0);
#else
...
static inline HANDLE winCreateFileW(const WCHAR *wfname, const DWORD mode, const DWORD creation)
{
...
#else
  return CreateFileW(wfname, mode, share, NULL, creation, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
} /* winCreateFileW */

static BOOL winSetFilePointer(HANDLE h, const PHYSFS_sint64 pos, PHYSFS_sint64 *_newpos,
                              const DWORD whence)
{
#if defined(PHYSFS_PLATFORM_WINRT) || (_WIN32_WINNT >= 0x0501)  // Windows XP+
  LARGE_INTEGER lipos;
...
static PHYSFS_sint64 winGetFileSize(HANDLE h)
{
  ...
  const BOOL rc = GetFileInformationByHandleEx(h, FileStandardInfo, &info, sizeof(info));
...
static LRESULT CALLBACK detectCDWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lparam)
{
  ...
  if (msg == WM_DESTROY)
    return 0;
           ...
           (lpdb->dbch_devicetype != DBT_DEVTYP_VOLUME) || ((lpdbv->dbcv_flags & DBTF_MEDIA) == 0))
  {
...
static DWORD WINAPI detectCDThread(LPVOID arg)
{
  ...
  detectCDHwnd =
      CreateWindowExA(0, classname, winname, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
...
static void detectAvailableCDs(PHYSFS_StringCallback cb, void *data)
{
  ...
  if (!detectCDThreadHandle)
  {
    ...
    detectCDThreadHandle = CreateThread(NULL, 0, detectCDThread, &initialDetectDone, 0, NULL);
    if (detectCDThreadHandle)
...
int __PHYSFS_platformInit(void) { return 1; /* It's all good */ } /* __PHYSFS_platformInit */
