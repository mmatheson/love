...
static int(_System *pUniUconvToUcs)(UconvObject, void **, size_t *, UniChar **, size_t *,
                                    size_t *) = NULL;
static int(_System *pUniUconvFromUcs)(UconvObject, UniChar **, size_t *, void **, size_t *,
                                      size_t *) = NULL;
...
static PHYSFS_ErrorCode errcodeFromAPIRET(const APIRET rc)
{
  switch (rc)
  {
    ...
    case ERROR_CANNOT_MAKE:
      return PHYSFS_ERR_IO; /* maybe this is wrong? */
    ...
    case ERROR_NEGATIVE_SEEK:
      return PHYSFS_ERR_INVALID_ARGUMENT;
...
static char *cvtUtf8ToCodepage(const char *utf8str)
{
  ...
  for (unilen = 0; uc2str[unilen]; unilen++)
  { /* spin */
  }
...
static char *cvtPathToCorrectCase(char *buf)
{
  ...
  while ((fname != NULL) && (*fname != '\0'))
  {
    ...
    *(fname - 1) = '\0'; /* isolate parent dir string. */
    ...
    rc = DosFindFirst(spec, &hdir, FILE_DIRECTORY, &fb, sizeof(fb), &count, FIL_STANDARD);
    if (rc == NO_ERROR)
    {
...
#define LOAD(x) (DosQueryProcAddr(uconvdll, 0, #x, (PFN *) &p##x) == NO_ERROR)
    ok = LOAD(UniCreateUconvObject) && LOAD(UniFreeUconvObject) && LOAD(UniUconvToUcs) &&
...
static int isCdRomDrive(ULONG drive)
{
  ...
  rc = DosOpen(
      drivename, &hfile, &ul1, 0, 0, OPEN_ACTION_OPEN_IF_EXISTS | OPEN_ACTION_FAIL_IF_NEW,
      OPEN_FLAGS_DASD | OPEN_FLAGS_FAIL_ON_ERROR | OPEN_FLAGS_NOINHERIT | OPEN_SHARE_DENYNONE,
      NULL);
  if (rc != NO_ERROR)
    return 0;
  ...
  rc = DosDevIOCtl(hfile, IOCTL_CDROMDISK, CDROMDISK_GETDRIVER, &param, sizeof(param), &ul1, &data,
                   sizeof(data), &ul2);
                                                          ...
                                                          const char *origdir, void *callbackdata)
{
  ...
  rc = DosFindFirst(cpspec, &hdir,
                    FILE_DIRECTORY | FILE_ARCHIVED | FILE_READONLY | FILE_HIDDEN | FILE_SYSTEM, &fb,
                    sizeof(fb), &count, FIL_STANDARD);
...
void *__PHYSFS_platformOpenRead(const char *filename)
{
   ...
   */
  return (void *) openFile(filename, OPEN_ACTION_OPEN_IF_EXISTS | OPEN_ACTION_FAIL_IF_NEW,
...
{
  return (void *) openFile(filename, OPEN_ACTION_REPLACE_IF_EXISTS | OPEN_ACTION_CREATE_IF_NEW,
                           OPEN_FLAGS_FAIL_ON_ERROR | OPEN_FLAGS_NO_LOCALITY |
...
void *__PHYSFS_platformOpenAppend(const char *filename)
{
   ...
   *  DosQueryFileInfo() will fail if we try to get a file length, etc.
   */
  hfile = openFile(filename, OPEN_ACTION_OPEN_IF_EXISTS | OPEN_ACTION_CREATE_IF_NEW,
                   OPEN_FLAGS_FAIL_ON_ERROR | OPEN_FLAGS_NO_LOCALITY | OPEN_FLAGS_NOINHERIT |
