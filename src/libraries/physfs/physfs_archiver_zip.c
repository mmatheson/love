...
static PHYSFS_uint32 zip_crypto_crc32(const PHYSFS_uint32 crc, const PHYSFS_uint8 val)
{
  ...
  for (i = 0; i < 8; i++) xorval = ((xorval & 1) ? (0xEDB88320 ^ (xorval >> 1)) : (xorval >> 1));
  return xorval ^ (crc >> 8);
} /* zip_crc32 */
...
static int zip_prep_crypto_keys(ZIPfileinfo *finfo, const PHYSFS_uint8 *crypto_header,
                                const PHYSFS_uint8 *password)
{
  ...
  const PHYSFS_uint8 verifier =
      (PHYSFS_uint8) ((usedate ? (entry->dos_mod_time >> 8) : (entry->crc >> 24)) & 0xFF);
  ...
  while (*password) zip_update_crypto_keys(keys, *(password++));
...
static PHYSFS_sint64 ZIP_read(PHYSFS_Io *_io, void *buf, PHYSFS_uint64 len)
{
  ...
  PHYSFS_sint64 avail = entry->uncompressed_size - finfo->uncompressed_position;
...
static const PHYSFS_Io ZIP_Io = {CURRENT_PHYSFS_IO_API_VERSION,
                                 NULL,
                                 ZIP_read,
                                 ZIP_write,
                                 ZIP_seek,
...
static int zip_parse_local(PHYSFS_Io *io, ZIPentry *entry)
{
  ...
  BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
  BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) && (ui32 != entry->compressed_size), PHYSFS_ERR_CORRUPT, 0);

  BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
  BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) && (ui32 != entry->uncompressed_size), PHYSFS_ERR_CORRUPT,
          0);
...
{
  return ((entry->resolved == ZIP_UNRESOLVED_SYMLINK) || (entry->resolved == ZIP_BROKEN_SYMLINK) ||
          (entry->symlink));
...
static inline int zip_has_symlink_attr(const ZIPentry *entry, const PHYSFS_uint32 extern_attr)
{
  PHYSFS_uint16 xattr = ((extern_attr >> 16) & 0xFFFF);
  return ((zip_version_does_symlinks(entry->version)) && (entry->uncompressed_size > 0) &&
...
static ZIPentry *zip_load_entry(ZIPinfo *info, const int zip64, const PHYSFS_uint64 ofs_fixup)
{
  ...
  {
    retval->resolved = (zip_has_symlink_attr(retval, external_attr)) ? ZIP_UNRESOLVED_SYMLINK
                                                                     : ZIP_UNRESOLVED_FILE;
  } /* else */
  ...
  if ((zip64) &&
      ((offset == 0xFFFFFFFF) || (starting_disk == 0xFFFFFFFF) ||
       (retval->compressed_size == 0xFFFFFFFF) || (retval->uncompressed_size == 0xFFFFFFFF)))
  {
    int found = 0;
    PHYSFS_uint16 sig = 0;
    PHYSFS_uint16 len = 0;
...
static int zip_load_entries(ZIPinfo *info, const PHYSFS_uint64 data_ofs,
                            const PHYSFS_uint64 central_ofs, const PHYSFS_uint64 entry_count)
{
  PHYSFS_Io *io = info->io;
...
static PHYSFS_sint64 zip64_find_end_of_central_dir(PHYSFS_Io *io, PHYSFS_sint64 _pos,
                                                   PHYSFS_uint64 offset)
{
  /*
...
static int zip64_parse_end_of_central_dir(ZIPinfo *info, PHYSFS_uint64 *data_start,
                                          PHYSFS_uint64 *dir_ofs, PHYSFS_uint64 *entry_count,
                                          PHYSFS_sint64 pos)
{
  PHYSFS_Io *io = info->io;
  PHYSFS_uint64 ui64;
...
static int zip_parse_end_of_central_dir(ZIPinfo *info, PHYSFS_uint64 *data_start,
                                        PHYSFS_uint64 *dir_ofs, PHYSFS_uint64 *entry_count)
{
  PHYSFS_Io *io = info->io;
  ...
  rc = zip64_parse_end_of_central_dir(info, data_start, dir_ofs, entry_count, pos - 20);
...
static void *ZIP_openArchive(PHYSFS_Io *io, const char *name, int forWriting, int *claimed)
{
  ZIPinfo *info = NULL;
  ZIPentry *root = NULL;
...
const PHYSFS_Archiver __PHYSFS_Archiver_ZIP = {
    ...
    {
        "ZIP", "PkZip/WinZip/Info-Zip compatible", "Ryan C. Gordon <icculus@icculus.org>",
        "https://icculus.org/physfs/", 1, /* supportsSymlinks */
    },
    ZIP_openArchive,
