...
#define __PHYSFS_ATOMIC_DECR(ptrval) _InterlockedDecrement((long *) (ptrval))
    ...
    (defined(__GNUC__) && (((__GNUC__ * 10000) + (__GNUC_MINOR__ * 100)) >= 40100))
...
#pragma aux _xadd_watcom = "lock xadd [ecx], eax" parm[ecx][eax] value[eax] modify exact[eax];
...
#define __PHYSFS_smallAlloc(bytes)                                                            \
  (__PHYSFS_initSmallAlloc(                                                                   \
      (((bytes) < __PHYSFS_SMALLALLOCTHRESHOLD) ? alloca((size_t) ((bytes) + sizeof(void *))) \
                                                : NULL),                                      \
      (bytes)))
...
#define __PHYSFS_ui64FitsAddressSpace(s)        \
  ...
  /*
   * Give a hash value for a C string (uses djb's xor hashing algorithm), case folding as it goes,
   *  assuming that this is only US-ASCII chars (one byte per char, only 'A' through 'Z' need
   * folding).
   */
...
#define allocator __PHYSFS_AllocatorHooks
  ...
  void *UNPK_addEntry(void *opaque, char *name, const int isdir, const PHYSFS_sint64 ctime,
                      const PHYSFS_sint64 mtime, const PHYSFS_uint64 pos, const PHYSFS_uint64 len);
...
#define UNPK_enumerate __PHYSFS_DirTreeEnumerate
  ...
  typedef struct __PHYSFS_DirTree
  {
    ...
    int only_usascii; /* non-zero to treat paths as US ASCII only (one byte per char, only 'A'
                         through 'Z' are considered for case folding). */
  } __PHYSFS_DirTree;
