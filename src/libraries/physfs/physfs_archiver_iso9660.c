...
static int iso9660LoadEntries(PHYSFS_Io *io, const int joliet, const char *base,
                              const PHYSFS_uint64 dirstart, const PHYSFS_uint64 dirend,
                              void *unpkarc);

static int iso9660AddEntry(PHYSFS_Io *io, const int joliet, const int isdir, const char *base,
                           PHYSFS_uint8 *fname, const int fnamelen, const PHYSFS_sint64 ts,
                           const PHYSFS_uint64 pos, const PHYSFS_uint64 len, void *unpkarc)
{
  char *fullpath;
  char *fnamecpy;
  size_t baselen;
                              ...
                              void *unpkarc)
{
  ...
  while (1)
  {
    ...
    if (!iso9660AddEntry(io, joliet, isdir, base, fname, fnamelen, timestamp, extent * 2048,
                         datalen, unpkarc))
    {
      return 0;
    } /* if */
...
static int parseVolumeDescriptor(PHYSFS_Io *io, PHYSFS_uint64 *_rootpos, PHYSFS_uint64 *_rootlen,
                                 int *_joliet, int *_claimed)
{
	...
	if (found < type)
	{
	  ...
	  *_rootlen = PHYSFS_swapULE32(datalen);
	  ...
	  if (found == 2) /* possible Joliet volume */
	  {
	    ...
	    *_joliet = !(flags & 1) && (s[0] == 0x25) && (s[1] == 0x2F) &&
...
static void *ISO9660_openArchive(PHYSFS_Io *io, const char *filename, int forWriting, int *claimed)
{
