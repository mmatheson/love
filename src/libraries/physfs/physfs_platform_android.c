...
char *__PHYSFS_platformCalcBaseDir(const char *argv0)
{
  ...
  if ((*jenv)->PushLocalFrame(jenv, 16) >= 0)
  {
    ...
    jmeth = (*jenv)->GetMethodID(jenv, (*jenv)->GetObjectClass(jenv, jcontext),
                                 "getPackageResourcePath", "()Ljava/lang/String;");
    jstr = (jstring) (*jenv)->CallObjectMethod(jenv, jcontext, jmeth);
    jexception =
        (*jenv)->ExceptionOccurred(jenv); /* this can't throw an exception, right? Just in case. */
    if (jexception != NULL)
    ...
    jmeth = (*jenv)->GetMethodID(jenv, (*jenv)->GetObjectClass(jenv, jcontext), "getFilesDir",
                                 "()Ljava/io/File;");
    jfileobj = (*jenv)->CallObjectMethod(jenv, jcontext, jmeth);
    if (jfileobj)
    {
      jmeth = (*jenv)->GetMethodID(jenv, (*jenv)->GetObjectClass(jenv, jfileobj),
  ...
  /* we can't return NULL because then PhysicsFS will treat argv0 as a string, but it's a non-NULL
   * jobject! */
  if (retval == NULL)
