...
static PHYSFS_sint64 nativeIo_write(PHYSFS_Io *io, const void *buffer, PHYSFS_uint64 len)
{
  NativeIoInfo *info = (NativeIoInfo *) io->opaque;
...
static const PHYSFS_Io __PHYSFS_nativeIoInterface = {CURRENT_PHYSFS_IO_API_VERSION,
                                                     NULL,
                                                     nativeIo_read,
                                                     nativeIo_write,
                                                     nativeIo_seek,
...
static PHYSFS_sint64 memoryIo_write(PHYSFS_Io *io, const void *buffer, PHYSFS_uint64 len)
{
...
static const PHYSFS_Io __PHYSFS_memoryIoInterface = {CURRENT_PHYSFS_IO_API_VERSION,
                                                     NULL,
                                                     memoryIo_read,
                                                     memoryIo_write,
                                                     memoryIo_seek,
...
static PHYSFS_sint64 handleIo_write(PHYSFS_Io *io, const void *buffer, PHYSFS_uint64 len)
{
...
static const PHYSFS_Io __PHYSFS_handleIoInterface = {CURRENT_PHYSFS_IO_API_VERSION,
                                                     NULL,
                                                     handleIo_read,
                                                     handleIo_write,
                                                     handleIo_seek,
...
static void __PHYSFS_quick_sort(void *a, size_t lo, size_t hi, int (*cmpfn)(void *, size_t, size_t),
                                void (*swapfn)(void *, size_t, size_t))
{
  size_t i;
  size_t j;
...
void __PHYSFS_sort(void *entries, size_t max, int (*cmpfn)(void *, size_t, size_t),
                   void (*swapfn)(void *, size_t, size_t))
{
  /*
...
static DirHandle *tryOpenDir(PHYSFS_Io *io, const PHYSFS_Archiver *funcs, const char *d,
                             int forWriting, int *_claimed)
{
...
static inline int sanitizePlatformIndependentPathWithRoot(const DirHandle *h, const char *src,
                                                          char *dst)
{
...
static DirHandle *createDirHandle(PHYSFS_Io *io, const char *newDir, const char *mountPoint,
                                  int forWriting)
{
...
PHYSFS_uint32 __PHYSFS_hashStringCaseFold(const char *str)
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      int i;
      for (i = 0; i < numbytes; i++) hash = ((hash << 5) + hash) ^ *(bytes++);
    } /* else */
  }   /* while */
...
void PHYSFS_freeList(void *list)
{
  ...
  if (list != NULL)
  {
    for (i = (void **) list; *i != NULL; i++) allocator.Free(*i);
...
int PHYSFS_setWriteDir(const char *newDir)
{
  ...
  if (writeDir != NULL)
  {
    BAIL_IF_MUTEX_ERRPASS(!freeDirHandle(writeDir, openWriteList), stateLock, 0);
    writeDir = NULL;
  } /* if */
...
static int doMount(PHYSFS_Io *io, const char *fname, const char *mountPoint, int appendToPath)
{
  DirHandle *dh;
  DirHandle *prev = NULL;
  DirHandle *i;
...
int PHYSFS_mountIo(PHYSFS_Io *io, const char *fname, const char *mountPoint, int appendToPath)
{
...
int PHYSFS_mountMemory(const void *buf, PHYSFS_uint64 len, void (*del)(void *), const char *fname,
...
int PHYSFS_mountHandle(PHYSFS_File *file, const char *fname, const char *mountPoint,
                       int appendToPath)
{
  int retval = 0;
  PHYSFS_Io *io = NULL;
...
int PHYSFS_unmount(const char *oldDir)
{
  ...
  for (i = searchPath; i != NULL; i = i->next)
  {
    ...
    {
      next = i->next;
      BAIL_IF_MUTEX_ERRPASS(!freeDirHandle(i, openReadList), stateLock, 0);
...
void PHYSFS_getSearchPathCallback(PHYSFS_StringCallback callback, void *data)
{
  ...
  for (i = searchPath; i != NULL; i = i->next) callback(data, i->dirName);
...
static PHYSFS_EnumerateCallbackResult setSaneCfgEnumCallback(void *_data, const char *dir,
                                                             const char *f)
{
...
int PHYSFS_setSaneConfig(const char *organization, const char *appName, const char *archiveExt,
                         int includeCdRoms, int archivesFirst)
{
  ...
  if (includeCdRoms)
  {
    ...
    char **i;
    for (i = cds; *i != NULL; i++) PHYSFS_mount(*i, NULL, 1);
    PHYSFS_freeList(cds);
  } /* if */
  ...
  while (1)
  {
	...
	exists = 0;
      ...
      retval = ((rc) && ((statbuf.filetype == PHYSFS_FILETYPE_DIRECTORY) ||
                         (statbuf.filetype == PHYSFS_FILETYPE_SYMLINK)));
    } /* if */
...
static PHYSFS_EnumerateCallbackResult enumFilesCallback(void *data, const char *origdir,
                                                        const char *str)
{
  PHYSFS_uint32 pos;
  void *ptr;
  ...
  if (pos != pecd->size)
  {
    memmove(&pecd->list[pos + 1], &pecd->list[pos], sizeof(char *) * ((pecd->size) - pos));
  } /* if */
...
char **PHYSFS_enumerateFiles(const char *path)
{
  ...
  if (!PHYSFS_enumerate(path, enumFilesCallback, &ecd))
  {
    ...
    PHYSFS_uint32 i;
    for (i = 0; i < ecd.size; i++) allocator.Free(ecd.list[i]);
 ...
 */
static PHYSFS_EnumerateCallbackResult enumerateFromMountPoint(DirHandle *i, const char *arcfname,
...
static PHYSFS_EnumerateCallbackResult enumCallbackFilterSymLinks(void *_data, const char *origdir,
                                                                 const char *fname)
{
...
int PHYSFS_enumerate(const char *_fn, PHYSFS_EnumerateCallback cb, void *data)
{
  ...
  else
  {
	...
	else if ((!allowSymLinks) && (i->funcs->info.supportsSymlinks))
	{
	  ...
	  retval = i->funcs->enumerate(i->opaque, arcfname, enumCallbackFilterSymLinks, _fn,
	                               &filterdata);
	  if (retval == PHYSFS_ENUM_ERROR)
	  {
	...
	{
	  retval = i->funcs->enumerate(i->opaque, arcfname, cb, _fn, data);
	} /* else */
      }   /* else if */
    }     /* for */
...
static PHYSFS_EnumerateCallbackResult enumFilesCallbackAlwaysSucceed(void *d, const char *origdir,
                                                                     const char *fname)
{
...
void PHYSFS_enumerateFilesCallback(const char *fname, PHYSFS_EnumFilesCallback callback, void *data)
{
...
PHYSFS_sint64 PHYSFS_read(PHYSFS_File *handle, void *buffer, PHYSFS_uint32 size,
                          PHYSFS_uint32 count)
{
...
static PHYSFS_sint64 doBufferedWrite(PHYSFS_File *handle, const void *buffer, const size_t len)
{
  FileHandle *fh = (FileHandle *) handle;
...
PHYSFS_sint64 PHYSFS_write(PHYSFS_File *handle, const void *buffer, PHYSFS_uint32 size,
                           PHYSFS_uint32 count)
{
  const PHYSFS_uint64 len = ((PHYSFS_uint64) size) * ((PHYSFS_uint64) count);
...
PHYSFS_sint64 PHYSFS_writeBytes(PHYSFS_File *handle, const void *buffer, PHYSFS_uint64 _len)
{
  const size_t len = (size_t) _len;
  FileHandle *fh = (FileHandle *) handle;
...
PHYSFS_sint64 PHYSFS_tell(PHYSFS_File *handle)
{
  ...
  const PHYSFS_sint64 retval =
      fh->forReading ? (pos - fh->buffill) + fh->bufpos : (pos + fh->buffill);
  return retval;
} /* PHYSFS_tell */
...
int __PHYSFS_DirTreeInit(__PHYSFS_DirTree *dt, const size_t entrylen, const int case_sensitive,
                         const int only_usascii)
{
...
{
  const PHYSFS_uint32 hashval = dt->case_sensitive ? __PHYSFS_hashString(name)
                                : dt->only_usascii ? __PHYSFS_hashStringCaseFoldUSAscii(name)
...
PHYSFS_EnumerateCallbackResult __PHYSFS_DirTreeEnumerate(void *opaque, const char *dname,
