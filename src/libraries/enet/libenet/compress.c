...
{
  /* only allocate enough symbols for reasonable MTUs, would need to be larger for large file
   * compression */
...
#define ENET_CONTEXT_RESCALE(context, minimum)                                        \
  {                                                                                   \
    (context)->total =                                                                \
        (context)->symbols ? enet_symbol_rescale((context) + (context)->symbols) : 0; \
...
#define ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, \
                            createRoot, visitNode, createRight, createLeft)                  \
  {                                                                                          \
    under_ = 0;                                                                              \
    count_ = minimum;                                                                        \
    if (!(context)->symbols)                                                                 \
...
#define ENET_CONTEXT_TRY_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, \
                                exclude)                                                         \
...
#define ENET_CONTEXT_ROOT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, \
                                 exclude)                                                         \
  ENET_CONTEXT_DECODE(                                                                            \
