...
static size_t commandSizes[ENET_PROTOCOL_COMMAND_COUNT] = {0,
    ...
    ENetPeer *peer, enet_uint16 reliableSequenceNumber, enet_uint8 channelID)
{
  ...
  commandNumber =
      (ENetProtocolCommand) (outgoingCommand->command.header.command & ENET_PROTOCOL_COMMAND_MASK);
...
static ENetPeer *enet_protocol_handle_connect(ENetHost *host, ENetProtocolHeader *header,
                                              ENetProtocol *command)
{
  ...
  if (host->outgoingBandwidth == 0 && peer->incomingBandwidth == 0)
    ...
    peer->windowSize =
        (ENET_MAX(host->outgoingBandwidth, peer->incomingBandwidth) / ENET_PEER_WINDOW_SIZE_SCALE) *
        ENET_PROTOCOL_MINIMUM_WINDOW_SIZE;
  else
    peer->windowSize =
        (ENET_MIN(host->outgoingBandwidth, peer->incomingBandwidth) / ENET_PEER_WINDOW_SIZE_SCALE) *
        ENET_PROTOCOL_MINIMUM_WINDOW_SIZE;
  ...
  if (host->incomingBandwidth == 0)
    ...
    windowSize =
        (host->incomingBandwidth / ENET_PEER_WINDOW_SIZE_SCALE) * ENET_PROTOCOL_MINIMUM_WINDOW_SIZE;
  ...
  *currentData += fragmentLength;
  ...
  if (startCommand == NULL)
  {
    startCommand = enet_peer_queue_incoming_command(
        peer, command, NULL, totalLength, ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT, fragmentCount);
    if (startCommand == NULL)
      return -1;
  }
                                                ...
                                                const ENetProtocol *command)
{
  ...
  if (peer->incomingBandwidth == 0 && host->outgoingBandwidth == 0)
    ...
    peer->windowSize =
        (ENET_MAX(peer->incomingBandwidth, host->outgoingBandwidth) / ENET_PEER_WINDOW_SIZE_SCALE) *
        ENET_PROTOCOL_MINIMUM_WINDOW_SIZE;
  else
    peer->windowSize =
        (ENET_MIN(peer->incomingBandwidth, host->outgoingBandwidth) / ENET_PEER_WINDOW_SIZE_SCALE) *
        ENET_PROTOCOL_MINIMUM_WINDOW_SIZE;
                                           ...
                                           const ENetProtocol *command)
{
  ...
  if (peer->state == ENET_PEER_STATE_CONNECTION_SUCCEEDED ||
      peer->state == ENET_PEER_STATE_DISCONNECTING || peer->state == ENET_PEER_STATE_CONNECTING)
    enet_protocol_dispatch_state(host, peer, ENET_PEER_STATE_ZOMBIE);
...
static int enet_protocol_send_reliable_outgoing_commands(ENetHost *host, ENetPeer *peer)
{
  ...
  while (currentCommand != enet_list_end(&peer->outgoingReliableCommands))
  {
	...
	enet_uint32 windowSize =
	    (peer->packetThrottle * peer->windowSize) / ENET_PEER_PACKET_THROTTLE_SCALE;
  ...
  while (host->continueSending)
...
#ifdef ENET_DEBUG
	printf(
	    "peer %u: %f%%+-%f%% packet loss, %u+-%u ms round trip time, %f%% throttle, %u/%u "
	    "outgoing, %u/%u incoming\n",
	...
	size_t originalSize = host->packetSize - sizeof(ENetProtocolHeader),
	       compressedSize = host->compressor.compress(
	           host->compressor.context, &host->buffers[1], host->bufferCount - 1, originalSize,
	...
	*checksum = host->checksum(host->buffers, host->bufferCount);
	...
	host->bufferCount = 2;
      ...
      sentLength =
          enet_socket_send(host->socket, &currentPeer->address, host->buffers, host->bufferCount);
...
/** Sends any queued packets on the host specified to its designated peers.
    ...
    @remarks this function need only be used in circumstances where one wishes to send queued
