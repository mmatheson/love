...
const char *Event::getName() const { return "love.event.sdl"; }
...
Message *Event::convert(const SDL_Event &e)
{
  ...
  switch (e.type)
  {
    ...
    case SDL_FINGERMOTION:
      ...
      if (touchNormalizationBug || fabs(touchinfo.x) >= 1.5 || fabs(touchinfo.y) >= 1.5 ||
          fabs(touchinfo.dx) >= 1.5 || fabs(touchinfo.dy) >= 1.5)
      {
...
Message *Event::convertJoystickEvent(const SDL_Event &e) const
{
  ...
  switch (e.type)
  {
	...
	break;
      ...
      msg = new Message((e.type == SDL_JOYBUTTONDOWN) ? "joystickpressed" : "joystickreleased",
                        vargs);
      break;
    case SDL_JOYAXISMOTION:
    {
    ...
    case SDL_CONTROLLERBUTTONDOWN:
    case SDL_CONTROLLERBUTTONUP:
      if (!joystick::sdl::Joystick::getConstant((SDL_GameControllerButton) e.cbutton.button,
                                                padbutton))
	break;
	...
	break;
      ...
      vargs.emplace_back(joysticktype, stick);
      vargs.emplace_back(txt, strlen(txt));
      msg = new Message(e.type == SDL_CONTROLLERBUTTONDOWN ? "gamepadpressed" : "gamepadreleased",
                        vargs);
      break;
    case SDL_CONTROLLERAXISMOTION:
