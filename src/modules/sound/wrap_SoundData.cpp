...
SoundData *luax_checksounddata(lua_State *L, int idx) { return luax_checktype<SoundData>(L, idx); }
...
{
  luax_markdeprecated(L, "SoundData:getChannels", API_METHOD, DEPRECATED_RENAMED,
                      "SoundData:getChannelCount");
  return w_SoundData_getChannelCount(L);
}

static const luaL_Reg w_SoundData_functions[] = {{"clone", w_SoundData_clone},
...
{
  int ret = luax_register_type(L, &SoundData::type, data::w_Data_functions, w_SoundData_functions,
                               nullptr);

  love::data::luax_rundatawrapper(L, SoundData::type);
  luax_runwrapper(L, sounddata_lua, sizeof(sounddata_lua), "SoundData.lua", SoundData::type,
                  nullptr);
