      ...
      channels(0)
{
  ...
  while (decoded > 0)
  {
    ...
    {
      while (bufferSize < size + decoded) bufferSize <<= 1;
...
SoundData *SoundData::clone() const { return new SoundData(*this); }
...
void *SoundData::getData() const { return (void *) data; }

size_t SoundData::getSize() const { return size; }
...
int SoundData::getSampleCount() const { return (int) ((size / channels) / (bitDepth / 8)); }
