...
Decoder *luax_checkdecoder(lua_State *L, int idx) { return luax_checktype<Decoder>(L, idx); }
...
int w_Decoder_decode(lua_State *L)
{
  ...
  if (decoded > 0)
  {
    luax_catchexcept(L,
                     [&]()
                     {
                       SoundData *s = instance()->newSoundData(
                           t->getBuffer(), decoded / (t->getBitDepth() / 8 * t->getChannelCount()),
...
{
  luax_markdeprecated(L, "Decoder:getChannels", API_METHOD, DEPRECATED_RENAMED,
                      "Decoder:getChannelCount");
  return w_Decoder_getChannelCount(L);
}

static const luaL_Reg w_Decoder_functions[] = {{"clone", w_Decoder_clone},
