...
int w_newDecoder(lua_State *L)
{
  ...
  Decoder *t = nullptr;
  luax_catchexcept(
      L, [&]() { t = instance()->newDecoder(data, bufferSize); }, [&](bool) { data->release(); });
...
int w_newSoundData(lua_State *L)
{
  ...
  if (lua_isnumber(L, 1))
  {
    ...
    luax_catchexcept(
        L, [&]() { t = instance()->newSoundData(samples, sampleRate, bitDepth, channels); });
  }
...
static const luaL_Reg functions[] = {
    {"newDecoder", w_newDecoder}, {"newSoundData", w_newSoundData}, {0, 0}};

static const lua_CFunction types[] = {luaopen_sounddata, luaopen_decoder, 0};
