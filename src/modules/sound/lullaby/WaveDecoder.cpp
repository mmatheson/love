...
WaveDecoder::~WaveDecoder() { wuff_close(handle); }
...
love::sound::Decoder *WaveDecoder::clone() { return new WaveDecoder(data.get(), bufferSize); }
...
bool WaveDecoder::isSeekable() { return true; }

int WaveDecoder::getChannelCount() const { return info.channels; }

int WaveDecoder::getBitDepth() const { return info.bits_per_sample == 8 ? 8 : 16; }

int WaveDecoder::getSampleRate() const { return info.sample_rate; }

double WaveDecoder::getDuration() { return (double) info.length / (double) info.sample_rate; }
