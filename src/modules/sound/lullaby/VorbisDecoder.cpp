...
static size_t vorbisRead(
    ...
    size_t byteSize /* how big a byte is*/, size_t sizeToRead /* How much we can read*/, void *datasource /* this is a pointer to the data we passed into ov_open_callbacks (our SOggFile struct*/)
{
  ...
  // Calculate how much we need to read.  This can be sizeToRead*byteSize or less depending on how
  // near the EOF marker we are
...
VorbisDecoder::~VorbisDecoder() { ov_clear(&handle); }
...
love::sound::Decoder *VorbisDecoder::clone() { return new VorbisDecoder(data.get(), bufferSize); }
...
int VorbisDecoder::decode()
{
  ...
  {
    long result = ov_read(&handle, (char *) buffer + size, bufferSize - size, endian,
                          (getBitDepth() == 16 ? 2 : 1), 1, 0);
...
int VorbisDecoder::getChannelCount() const { return vorbisInfo->channels; }

int VorbisDecoder::getBitDepth() const { return 16; }

int VorbisDecoder::getSampleRate() const { return (int) vorbisInfo->rate; }
