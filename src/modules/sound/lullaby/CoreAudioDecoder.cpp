...
namespace
{
OSStatus readFunc(void *inClientData, SInt64 inPosition, UInt32 requestCount, void *buffer,
                  UInt32 *actualCount)
{
      ...
      duration(-2.0)
{
  try
  {
    ...
    err = AudioFileOpenWithCallbacks(data, readFunc, nullptr, getSizeFunc, nullptr,
                                     kAudioFileMP3Type, &audioFile);
    if (err != noErr)
    ...
    err = ExtAudioFileGetProperty(extAudioFile, kExtAudioFileProperty_FileDataFormat, &propertySize,
                                  &inputInfo);
    ...
    err = ExtAudioFileSetProperty(extAudioFile, kExtAudioFileProperty_ClientDataFormat,
                                  propertySize, &outputInfo);
...
CoreAudioDecoder::~CoreAudioDecoder() { closeAudioFile(); }
...
bool CoreAudioDecoder::accepts(const std::string &ext)
{
  ...
  OSStatus err = AudioFileGetGlobalInfoSize(kAudioFileGlobalInfo_ReadableTypes, sizeof(UInt32),
                                            nullptr, &size);
  if (err != noErr)
    return false;
  ...
  for (UInt32 type : types)
  {
    ...
    err = AudioFileGetGlobalInfo(kAudioFileGlobalInfo_ExtensionsForType, sizeof(UInt32), &type,
                                 &size, &exts);
    if (err != noErr)
      continue;
...
bool CoreAudioDecoder::isSeekable() { return true; }

int CoreAudioDecoder::getChannelCount() const { return outputInfo.mChannelsPerFrame; }

int CoreAudioDecoder::getBitDepth() const { return outputInfo.mBitsPerChannel; }
...
double CoreAudioDecoder::getDuration()
{
  ...
  if (duration == -2.0)
  {
    ...
    OSStatus err = ExtAudioFileGetProperty(extAudioFile, kExtAudioFileProperty_FileLengthFrames,
                                           &psize, &samples);
