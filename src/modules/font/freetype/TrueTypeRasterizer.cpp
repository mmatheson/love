...
TrueTypeRasterizer::TrueTypeRasterizer(FT_Library library, love::Data *data, int size,
                                       float dpiscale, Hinting hinting)
    : data(data),
      hinting(hinting)
{
...
TrueTypeRasterizer::~TrueTypeRasterizer() { FT_Done_Face(face); }

int TrueTypeRasterizer::getLineHeight() const { return (int) (getHeight() * 1.25); }
...
int TrueTypeRasterizer::getGlyphCount() const { return (int) face->num_glyphs; }
...
{
  FT_Vector kerning = {};
  FT_Get_Kerning(face, FT_Get_Char_Index(face, leftglyph), FT_Get_Char_Index(face, rightglyph),
                 FT_KERNING_DEFAULT, &kerning);
  return float(kerning.x >> 6);
}

Rasterizer::DataType TrueTypeRasterizer::getDataType() const { return DATA_TRUETYPE; }
