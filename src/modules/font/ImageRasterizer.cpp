...
int ImageRasterizer::getLineHeight() const { return getHeight(); }
...
GlyphData *ImageRasterizer::getGlyphData(uint32 glyph) const
{
  ...
  {
    Color32 p =
        imagepixels[it->second.x + (i % gm.width) + (imageData->getWidth() * (i / gm.width))];
...
void ImageRasterizer::load()
{
  ...
  for (int i = 0; i < numglyphs; ++i)
  {
    ...
    while (start < imgw && pixels[start] == spacer) ++start;

    end = start;

    // Find where glyph ends.
    while (end < imgw && pixels[end] != spacer) ++end;
...
int ImageRasterizer::getGlyphCount() const { return numglyphs; }
...
Rasterizer::DataType ImageRasterizer::getDataType() const { return DATA_IMAGE; }
