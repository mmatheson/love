...
int w_newRasterizer(lua_State *L)
{
  ...
  else if (lua_isnoneornil(L, 2))
  {
    ...
    luax_catchexcept(
        L, [&]() { t = instance()->newRasterizer(d); }, [&](bool) { d->release(); });
...
int w_newTrueTypeRasterizer(lua_State *L)
{
  ...
  if (lua_type(L, 1) == LUA_TNUMBER || lua_isnone(L, 1))
  {
    ...
    if (hintstr && !TrueTypeRasterizer::getConstant(hintstr, hinting))
      return luax_enumerror(L, "TrueType font hinting mode",
    ...
    else
    {
      ...
      luax_catchexcept(L,
                       [&]() { t = instance()->newTrueTypeRasterizer(size, dpiscale, hinting); });
    }
  }
  else
  {
    ...
    {
      luax_catchexcept(
          L, [&]() { t = instance()->newTrueTypeRasterizer(d, size, hinting); },
          [&](bool) { d->release(); });
    }
    else
    {
      ...
      luax_catchexcept(
          L, [&]() { t = instance()->newTrueTypeRasterizer(d, size, dpiscale, hinting); },
          [&](bool) { d->release(); });
    }
  }
...
{
  if (lua_type(L, 1) == LUA_TSTRING || luax_istype(L, idx, love::filesystem::File::type) ||
      luax_istype(L, idx, love::filesystem::FileData::type))
...
int w_newBMFontRasterizer(lua_State *L)
{
  ...
  luax_catchexcept(
      L, [&]() { t = instance()->newBMFontRasterizer(d, images, dpiscale); },
      [&](bool)
      {
        d->release();
        for (auto id : images) id->release();
      });
...
int w_newImageRasterizer(lua_State *L)
{
  ...
  luax_catchexcept(
      L, [&]() { t = instance()->newImageRasterizer(d, glyphs, extraspacing, dpiscale); });
...
static const luaL_Reg functions[] = {{"newRasterizer", w_newRasterizer},
...
static const lua_CFunction types[] = {luaopen_glyphdata, luaopen_rasterizer, 0};
