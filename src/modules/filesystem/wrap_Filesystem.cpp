...
bool luax_cangetfiledata(lua_State *L, int idx)
{
  return lua_isstring(L, idx) || luax_istype(L, idx, File::type) ||
         luax_istype(L, idx, FileData::type);
}
...
int w_write(lua_State *L) { return w_write_or_append(L, File::MODE_WRITE); }

int w_append(lua_State *L) { return w_write_or_append(L, File::MODE_APPEND); }
...
int w_setRequirePath(lua_State *L)
{
  ...
  while (std::getline(path, element, ';')) requirePath.push_back(element);
...
int extloader(lua_State *L)
{
  ...
  if (!handle)
  {
	...
	std::string filepath =
	    inst->getRealDirectory(element.c_str()) + LOVE_PATH_SEPARATOR + element;
...
{
  luax_markdeprecated(L, "love.filesystem.exists", API_FUNCTION, DEPRECATED_REPLACED,
...
{
  luax_markdeprecated(L, "love.filesystem.isDirectory", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.filesystem.getInfo");
  const char *arg = luaL_checkstring(L, 1);
  Filesystem::Info info = {};
...
{
  luax_markdeprecated(L, "love.filesystem.isFile", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.filesystem.getInfo");
  const char *arg = luaL_checkstring(L, 1);
  Filesystem::Info info = {};
  bool exists = instance()->getInfo(arg, info);
...
{
  luax_markdeprecated(L, "love.filesystem.isSymlink", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.filesystem.getInfo");
  const char *filename = luaL_checkstring(L, 1);
  Filesystem::Info info = {};
...
{
  luax_markdeprecated(L, "love.filesystem.getLastModified", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.filesystem.getInfo");
...
{
  luax_markdeprecated(L, "love.filesystem.getSize", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.filesystem.getInfo");
...
static const luaL_Reg functions[] = {{"init", w_init},
                                     {"setFused", w_setFused},
                                     {"isFused", w_isFused},
...
static const lua_CFunction types[] = {luaopen_file, luaopen_droppedfile, luaopen_filedata, 0};
