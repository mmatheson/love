...
const char *Filesystem::getName() const { return "love.filesystem.physfs"; }
...
bool Filesystem::setIdentity(const char *ident, bool appendToPath)
{
  ...
  save_path_relative =
      std::string(LOVE_APPDATA_PREFIX LOVE_APPDATA_FOLDER LOVE_PATH_SEPARATOR) + save_identity;
...
const char *Filesystem::getIdentity() const { return save_identity.c_str(); }
...
bool Filesystem::setSource(const char *source)
{
  ...
  if (!isAAssetMounted)
  {
    ...
    if (!PHYSFS_mount(new_search_path.c_str(), nullptr, 1))
    {
      ...
      if (!love::android::loadGameArchiveToMemory(new_search_path.c_str(), &game_archive_ptr,
                                                  &game_archive_size))
      {
	SDL_Log("Failure memory loading archive %s", new_search_path.c_str());
	return false;
      }
      if (!PHYSFS_mountMemory(game_archive_ptr, game_archive_size,
...
const char *Filesystem::getSource() const { return game_source.c_str(); }
...
const char *Filesystem::getSaveDirectory() { return save_path_full.c_str(); }
...
{
  if (std::find(allowedMountPaths.begin(), allowedMountPaths.end(), path) ==
      allowedMountPaths.end())
