...
int w_noise(lua_State *L)
{
  ...
  for (int i = 0; i < nargs; i++) args[i] = (float) luaL_checknumber(L, i + 1);
...
int w_compress(lua_State *L)
{
  ...
  luax_markdeprecated(L, "love.math.compress", API_FUNCTION, DEPRECATED_REPLACED,
                      "love.data.compress");
...
{
  using namespace love::data;
  luax_markdeprecated(L, "love.math.decompress", API_FUNCTION, DEPRECATED_REPLACED,
...
static const lua_CFunction types[] = {luaopen_randomgenerator, luaopen_beziercurve,
                                      luaopen_transform, 0};
