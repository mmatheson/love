...
int w_Transform_setMatrix(lua_State *L)
{
  ...
  else
  {
    if (columnmajor)
    {
      for (int i = 0; i < 16; i++) elements[i] = (float) luaL_checknumber(L, idx + i);
    }
    else
    {
      for (int column = 0; column < 4; column++)
...
int w_Transform_getMatrix(lua_State *L)
{
  ...
  for (int row = 0; row < 4; row++)
  {
    for (int column = 0; column < 4; column++) lua_pushnumber(L, elements[column * 4 + row]);
  }
...
static const luaL_Reg functions[] = {{"clone", w_Transform_clone},
