...
void subdivide(vector<love::Vector2> &points, int k)
{
  ...
  for (size_t step = 1; step < points.size(); ++step)
  {
    ...
    for (size_t i = 0; i < points.size() - step; ++i) points[i] = (points[i] + points[i + 1]) * .5;
  }
  left.push_back(points[0]);
  ...
  for (size_t i = 0; i < left.size(); ++i) points[i] = left[i];
...
const Vector2 &BezierCurve::getControlPoint(int i) const
{
  ...
  while (i < 0) i += controlPoints.size();

  while ((size_t) i >= controlPoints.size()) i -= controlPoints.size();
...
{
  for (size_t i = 0; i < controlPoints.size(); ++i) controlPoints[i] += t;
}
...
BezierCurve *BezierCurve::getSegment(double t1, double t2) const
{
  ...
  for (size_t step = 1; step < left.size(); ++step)
  {
    ...
    for (size_t i = 0; i < left.size() - step; ++i) left[i] += (left[i + 1] - left[i]) * s;
  }
