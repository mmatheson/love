...
void Window::setGraphics(graphics::Graphics *graphics) { this->graphics.set(graphics); }
                                    ...
                                    bool stencil, int depth)
{
  ...
  if (!context || !window)
  {
    ...
    std::string message =
        "This program requires a graphics card and video drivers which support OpenGL 2.1 or "
        "OpenGL ES 2.";
...
bool Window::setFullscreen(bool fullscreen, Window::FullscreenType fstype)
{
  ...
  if (graphics.get() && graphics->isCanvasActive())
    throw love::Exception(
        "love.window.setFullscreen cannot be called while a Canvas is active in love.graphics.");
...
bool Window::setFullscreen(bool fullscreen) { return setFullscreen(fullscreen, settings.fstype); }
...
bool Window::setIcon(love::image::ImageData *imgd)
{
...

  {
    ...
    sdlicon = SDL_CreateRGBSurfaceFrom(imgd->getData(), w, h, bytesperpixel * 8, pitch, rmask,
                                       gmask, bmask, amask);
  }
...
love::image::ImageData *Window::getIcon() { return icon.get(); }
...
int Window::getVSync() const { return context != nullptr ? SDL_GL_GetSwapInterval() : 0; }
...
bool Window::isDisplaySleepEnabled() const { return SDL_IsScreenSaverEnabled() != SDL_FALSE; }
...
void Window::swapBuffers()
{
  ...
  // VSync can interact badly with Windows desktop composition (DWM) in windowed mode. DwmFlush can
  // be used instead of vsync, but it's much less flexible so we're very conservative here with
  // where it's used:
  // - It won't work with exclusive or desktop fullscreen.
  // - DWM refreshes don't always match the refresh rate of the monitor the window is in (or the
  // requested swap
  ...
  {
    // Desktop composition is always enabled in Windows 8+. But DwmIsCompositionEnabled won't always
    // return true... (see DwmIsCompositionEnabled docs).
	...
	dwmRefreshRate =
	    (double) info.rateRefresh.uiNumerator / (double) info.rateRefresh.uiDenominator;
	...
	SDL_GetCurrentDisplayMode(displayindex, &dmode);
      ...
      if (dmode.refresh_rate > 0 && dwmRefreshRate > 0 &&
          (fabs(dmode.refresh_rate - dwmRefreshRate) < 2))
      {
...
bool Window::hasMouseFocus() const { return (window && SDL_GetMouseFocus() == window); }
...
int Window::getPixelWidth() const { return pixelWidth; }

int Window::getPixelHeight() const { return pixelHeight; }
...
double Window::getDPIScale() const { return settings.usedpiscale ? getNativeDPIScale() : 1.0; }
...
double Window::toPixels(double x) const { return x * getDPIScale(); }
...
double Window::fromPixels(double x) const { return x / getDPIScale(); }
...
const void *Window::getHandle() const { return window; }
...
const char *Window::getName() const { return "love.window.sdl"; }
