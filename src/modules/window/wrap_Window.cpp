...
static int readWindowSettings(lua_State *L, int idx, WindowSettings &settings)
{
  ...
  settings.fullscreen =
      luax_boolflag(L, idx, settingName(Window::SETTING_FULLSCREEN), settings.fullscreen);
  ...
  settings.resizable =
      luax_boolflag(L, idx, settingName(Window::SETTING_RESIZABLE), settings.resizable);
  settings.minwidth =
      luax_intflag(L, idx, settingName(Window::SETTING_MIN_WIDTH), settings.minwidth);
  settings.minheight =
      luax_intflag(L, idx, settingName(Window::SETTING_MIN_HEIGHT), settings.minheight);
  settings.borderless =
      luax_boolflag(L, idx, settingName(Window::SETTING_BORDERLESS), settings.borderless);
  settings.centered =
      luax_boolflag(L, idx, settingName(Window::SETTING_CENTERED), settings.centered);
  settings.display =
      luax_intflag(L, idx, settingName(Window::SETTING_DISPLAY), settings.display + 1) - 1;
  settings.highdpi = luax_boolflag(L, idx, settingName(Window::SETTING_HIGHDPI), settings.highdpi);
  settings.usedpiscale =
      luax_boolflag(L, idx, settingName(Window::SETTING_USE_DPISCALE), settings.usedpiscale);
...
int w_setMode(lua_State *L)
{
  ...
  luax_catchexcept(L, [&]() { luax_pushboolean(L, instance()->setWindow(w, h, &settings)); });
...
int w_showMessageBox(lua_State *L)
{
  ...
  else
  {
    ...
    bool success =
        instance()->showMessageBox(data.title, data.message, data.type, data.attachToWindow);
    luax_pushboolean(L, success);
  }
...
static const luaL_Reg functions[] = {{"getDisplayCount", w_getDisplayCount},
