...
int w_newImageData(lua_State *L)
{
  ...
  if (lua_isnumber(L, 1))
  {
	...
	t->release();
	return luaL_error(
	    L, "The size of the raw byte string must match the ImageData's actual size in bytes.");
      }
  ...
  else if (filesystem::luax_cangetdata(L, 1))  // Case 2: File(Data).
  {
    ...
    ImageData *t = nullptr;
    luax_catchexcept(
        L, [&]() { t = instance()->newImageData(data); }, [&](bool) { data->release(); });
...
int w_newCompressedData(lua_State *L)
{
  ...
  luax_catchexcept(
      L, [&]() { t = instance()->newCompressedData(data); }, [&](bool) { data->release(); });
...
static const lua_CFunction types[] = {luaopen_imagedata, luaopen_compressedimagedata, 0};
