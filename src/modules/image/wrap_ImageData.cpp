...
ImageData *luax_checkimagedata(lua_State *L, int idx) { return luax_checktype<ImageData>(L, idx); }
...
static const luaL_Reg w_ImageData_functions[] = {{"clone", w_ImageData_clone},
...
{
  int ret = luax_register_type(L, &ImageData::type, data::w_Data_functions, w_ImageData_functions,
                               nullptr);

  love::data::luax_rundatawrapper(L, ImageData::type);
  luax_runwrapper(L, imagedata_lua, sizeof(imagedata_lua), "ImageData.lua", ImageData::type,
                  &ffifuncs);
