...
love::image::ImageData *ImageData::clone() const { return new ImageData(*this); }
...
void ImageData::decode(Data *data)
{
  ...
  if (decodedimage.data == nullptr)
  {
    ...
    if (filedata != nullptr)
    {
      ...
      throw love::Exception("Could not decode file '%s' to ImageData: unsupported file format",
                            name.c_str());
    }
    else
  ...
  if (decodedimage.size !=
      decodedimage.width * decodedimage.height * getPixelFormatSize(decodedimage.format))
  {
...
love::filesystem::FileData *ImageData::encode(FormatHandler::EncodedFormat encodedFormat,
...
love::thread::Mutex *ImageData::getMutex() const { return mutex; }
...
bool ImageData::canPaste(PixelFormat src, PixelFormat dst)
{
  ...
  if (!(src == PIXELFORMAT_RGBA8 || src == PIXELFORMAT_RGBA16 || src == PIXELFORMAT_RGBA16F ||
        src == PIXELFORMAT_RGBA32F))
    return false;

  if (!(dst == PIXELFORMAT_RGBA8 || dst == PIXELFORMAT_RGBA16 || dst == PIXELFORMAT_RGBA16F ||
        dst == PIXELFORMAT_RGBA32F))
    return false;
...
StringMap<FormatHandler::EncodedFormat, FormatHandler::ENCODED_MAX_ENUM> ImageData::encodedFormats(
    ImageData::encodedFormatEntries, sizeof(ImageData::encodedFormatEntries));
