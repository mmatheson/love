...
static_assert(sizeof(KTXHeader) == KTX_HEADER_SIZE,
              "Real size of KTX header doesn't match struct size!");
...
StrongRef<CompressedMemory> KTXHandler::parseCompressed(
    ...
    bool &sRGB)
{
  ...
  if (header.endianness == KTX_ENDIAN_REF_REV)
  {
    ...
    for (int i = 0; i < 12; i++) headerArray[i] = swapuint32(headerArray[i]);
  }
