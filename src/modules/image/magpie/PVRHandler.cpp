...
size_t getMipLevelSize(const PVRTexHeaderV3 &header, int miplevel)
{
  ...
  if (header.pixelFormat >= ePVRTPF_ASTC_4x4 && header.pixelFormat <= ePVRTPF_ASTC_12x12)
    return (width / smallestwidth) * (height / smallestheight) * (depth / smallestdepth) *
           (128 / 8);
  else
...
StrongRef<CompressedMemory> PVRHandler::parseCompressed(
    ...
    bool &sRGB)
{
  ...
  for (int i = 0; i < (int) header3.numMipmaps; i++) totalsize += getMipLevelSize(header3, i);
