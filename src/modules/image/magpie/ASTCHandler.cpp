...
bool ASTCHandler::canParseCompressed(Data *data)
{
  ...
  uint32 identifier = (uint32) header->identifier[0] + ((uint32) header->identifier[1] << 8) +
...
StrongRef<CompressedMemory> ASTCHandler::parseCompressed(
    ...
    bool &sRGB)
{
  ...
  images.emplace_back(new CompressedSlice(cformat, sizeX, sizeY, memory, 0, totalsize),
                      Acquire::NORETAIN);
