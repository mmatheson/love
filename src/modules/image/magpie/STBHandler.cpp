...
bool STBHandler::canDecode(Data *data)
{
  ...
  int status = stbi_info_from_memory((const stbi_uc *) data->getData(), (int) data->getSize(), &w,
                                     &h, &comp);
...
FormatHandler::DecodedImage STBHandler::decode(Data *data)
{
  ...
  {
    img.data = (unsigned char *) stbi_loadf_from_memory(buffer, bufferlen, &img.width, &img.height,
                                                        &comp, 4);
