...
PNGHandler::DecodedImage PNGHandler::decode(Data *fdata)
{
  ...
  if (state.info_raw.bitdepth == 16)
  {
    ...
    for (size_t i = 0; i < numpixelcomponents; i++) pixeldata[i] = swapuint16(pixeldata[i]);
  }
#endif
...
FormatHandler::EncodedImage PNGHandler::encode(const DecodedImage &img, EncodedFormat encodedFormat)
{
  ...
  if (state.info_raw.bitdepth == 16)
  {
    ...
    for (size_t i = 0; i < numpixelcomponents; i++) swappeddata[i] = swapuint16(rawdata[i]);

    data = (const uint8 *) swappeddata;
  }
#endif

  unsigned status = lodepng_encode(&encimg.data, &encimg.size, data, img.width, img.height, &state);
