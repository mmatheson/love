  ...
  luax_catchexcept(L,
                   [&]()
                   {
                     if (lua_gettop(L) >= 8)
                     {
                       ...
                       j = instance()->newRevoluteJoint(body1, body2, xA, yA, xB, yB,
  ...
  luax_catchexcept(L,
                   [&]()
                   {
                     if (lua_gettop(L) >= 10)
                     {
                       ...
                       j = instance()->newPrismaticJoint(body1, body2, xA, yA, xB, yB, ax, ay,
                                                         collideConnected, referenceAngle);
                     }
                     else
                       j = instance()->newPrismaticJoint(body1, body2, xA, yA, xB, yB, ax, ay,
...
int w_newPulleyJoint(lua_State *L)
{
  ...
  bool collideConnected = luax_optboolean(
      L, 12, true);  // PulleyJoints default to colliding connected bodies, see b2PulleyJoint.h

  PulleyJoint *j;
  luax_catchexcept(L,
                   [&]()
                   {
                     j = instance()->newPulleyJoint(body1, body2, b2Vec2(gx1, gy1),
                                                    b2Vec2(gx2, gy2), b2Vec2(x1, y1),
...
int w_newGearJoint(lua_State *L)
{
  ...
  GearJoint *j;
  luax_catchexcept(
      L, [&]() { j = instance()->newGearJoint(joint1, joint2, ratio, collideConnected); });
  luax_pushtype(L, j);
  j->release();
  return 1;
}
...
int w_getDistance(lua_State *L) { return instance()->getDistance(L); }
...
static const lua_CFunction types[] = {luaopen_world,          luaopen_contact,
                                      luaopen_body,           luaopen_fixture,
                                      luaopen_shape,          luaopen_circleshape,
                                      luaopen_polygonshape,   luaopen_edgeshape,
                                      luaopen_chainshape,     luaopen_joint,
                                      luaopen_mousejoint,     luaopen_distancejoint,
                                      luaopen_prismaticjoint, luaopen_revolutejoint,
                                      luaopen_pulleyjoint,    luaopen_gearjoint,
                                      luaopen_frictionjoint,  luaopen_weldjoint,
                                      luaopen_wheeljoint,     luaopen_ropejoint,
                                      luaopen_motorjoint,     0};
