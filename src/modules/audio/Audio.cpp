...
void setRequestRecordingPermission(bool rec) { requestRecPermission = rec; }

bool getRequestRecordingPermission() { return requestRecPermission; }
...
StringMap<Audio::DistanceModel, Audio::DISTANCE_MAX_ENUM> Audio::distanceModels(
    Audio::distanceModelEntries, sizeof(Audio::distanceModelEntries));

bool Audio::getConstant(const char *in, DistanceModel &out) { return distanceModels.find(in, out); }

bool Audio::getConstant(DistanceModel in, const char *&out) { return distanceModels.find(in, out); }

std::vector<std::string> Audio::getConstants(DistanceModel) { return distanceModels.getNames(); }
