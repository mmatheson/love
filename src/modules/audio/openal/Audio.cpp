      ...
      distanceModel(DISTANCE_INVERSE_CLAMPED)
{
  ...
  alcDevicePauseSOFT = hasPauseDeviceExt
                           ...
                           : nullptr;
  alcDeviceResumeSOFT =
      hasPauseDeviceExt ? (LPALCDEVICERESUMESOFT) alcGetProcAddress(device, "alcDeviceResumeSOFT")
                        : nullptr;
#endif
}
...
const char *Audio::getName() const { return "love.audio.openal"; }
...
int Audio::getActiveSourceCount() const { return pool->getActiveSourceCount(); }

int Audio::getMaxSources() const { return pool->getMaxSources(); }

bool Audio::play(love::audio::Source *source) { return source->play(); }
...
void Audio::stop(love::audio::Source *source) { source->stop(); }
...
void Audio::stop() { return Source::stop(pool); }

void Audio::pause(love::audio::Source *source) { source->pause(); }
...
std::vector<love::audio::Source *> Audio::pause() { return Source::pause(pool); }
...
void Audio::pauseContext()
{
  ...
  else
  {
    ...
    // since 1.16
    for (auto &src : pausedSources) src->release();
    pausedSources = pause();
    for (auto &src : pausedSources) src->retain();
  }
#else
  alcMakeContextCurrent(nullptr);
#endif
...
void Audio::setVolume(float volume) { alListenerf(AL_GAIN, volume); }
...
void Audio::setPosition(float *v) { alListenerfv(AL_POSITION, v); }

void Audio::getOrientation(float *v) const { alGetListenerfv(AL_ORIENTATION, v); }

void Audio::setOrientation(float *v) { alListenerfv(AL_ORIENTATION, v); }

void Audio::getVelocity(float *v) const { alGetListenerfv(AL_VELOCITY, v); }

void Audio::setVelocity(float *v) { alListenerfv(AL_VELOCITY, v); }
...
*/
Audio::DistanceModel Audio::getDistanceModel() const { return distanceModel; }
...
const std::vector<love::audio::RecordingDevice *> &Audio::getRecordingDevices()
{
  ...
  for (auto c : capture) c->release();
  capture.clear();
  capture.reserve(devices.size());

  // this needs to be executed in specific order
  for (unsigned int i = 0; i < devnames.size(); i++) capture.push_back(devices[i]);
...
bool Audio::getActiveEffects(std::vector<std::string> &list) const
{
  ...
  for (auto i : effectmap) list.push_back(i.first);

  return true;
}

int Audio::getMaxSceneEffects() const { return MAX_SCENE_EFFECTS; }

int Audio::getMaxSourceEffects() const { return MAX_SOURCE_EFFECTS; }
...
void Audio::initializeEFX()
{
  ...
  if (!alGenEffects || !alDeleteEffects || !alIsEffect || !alGenFilters || !alDeleteFilters ||
      !alIsFilter || !alGenAuxiliaryEffectSlots || !alDeleteAuxiliaryEffectSlots ||
      !alIsAuxiliaryEffectSlot || !alEffecti || !alEffectiv || !alEffectf || !alEffectfv ||
      !alGetEffecti || !alGetEffectiv || !alGetEffectf || !alGetEffectfv || !alFilteri ||
      !alFilteriv || !alFilterf || !alFilterfv || !alGetFilteri || !alGetFilteriv ||
      !alGetFilterf || !alGetFilterfv || !alAuxiliaryEffectSloti || !alAuxiliaryEffectSlotiv ||
      !alAuxiliaryEffectSlotf || !alAuxiliaryEffectSlotfv || !alGetAuxiliaryEffectSloti ||
