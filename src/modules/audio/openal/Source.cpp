...
class SpatialSupportException : public love::Exception
{
 public:
  SpatialSupportException()
      : Exception(
            "This spatial audio functionality is only available for mono Sources. \
...
StaticDataBuffer::~StaticDataBuffer() { alDeleteBuffers(1, &buffer); }
      ...
      bitDepth(soundData->getBitDepth())
{
  ...
  staticBuffer.set(
      new StaticDataBuffer(fmt, soundData->getData(), (ALsizei) soundData->getSize(), sampleRate),
      Acquire::NORETAIN);
  ...
  for (int i = 0; i < audiomodule()->getMaxSourceEffects(); i++) slotlist.push(i);
}
...
love::audio::Source *Source::clone() { return new Source(*this); }
...
void Source::seek(double offset, Source::Unit unit)
{
  ...
  if (wasPlaying &&
      (alGetError() == AL_INVALID_VALUE || (sourceType == TYPE_STREAM && !isPlaying())))
  {
    stop();
    if (isLooping())
      play();
...
bool Source::queue(void *data, size_t length, int dataSampleRate, int dataBitDepth,
                   int dataChannels)
{
  if (sourceType != TYPE_QUEUE)
...
void Source::teardownAtomic()
{
  switch (sourceType)
  {
    ...
    case TYPE_STREAM:
    {
      ...
      for (int i = 0; i < queued; i++) unusedBuffers.push(buffers[i]);
      break;
    }
    case TYPE_QUEUE:
    {
...
void Source::resumeAtomic()
{
  if (valid && !isPlaying())
  {
    ...
    if (alGetError() == AL_INVALID_VALUE ||
        (sourceType == TYPE_STREAM && (int) unusedBuffers.size() == buffers))
      stop();
  }
}
...
std::vector<love::audio::Source *> Source::pause(Pool *pool)
{
  ...
  auto newend = std::remove_if(sources.begin(), sources.end(),
                               [](love::audio::Source *s) { return !s->isPlaying(); });
...
void Source::reset()
{
  ...
  for (auto i : effectmap)
    alSource3i(source, AL_AUXILIARY_SEND_FILTER, i.second.target, i.second.slot,
...
int Source::getChannelCount() const { return channels; }
