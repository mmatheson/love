...
Effect::~Effect() { deleteEffect(); }

Effect *Effect::clone() { return new Effect(*this); }
...
ALuint Effect::getEffect() const { return effect; }
...
#define PARAMSTR(i, e, v) \
  ...
  switch (type)
  {
    ...
    {
      Waveform wave =
          static_cast<Waveform>(getValue(CHORUS_WAVEFORM, static_cast<int>(WAVE_MAX_ENUM)));
      if (wave == WAVE_SINE)
    ...
    {
      Waveform wave =
          static_cast<Waveform>(getValue(FLANGER_WAVEFORM, static_cast<int>(WAVE_MAX_ENUM)));
      if (wave == WAVE_SINE)
	...
	alEffecti(effect, AL_FLANGER_WAVEFORM, AL_FLANGER_DEFAULT_WAVEFORM);
      ...
      /*
              case TYPE_FREQSHIFTER:
              {
                      ...
                      Direction dir = static_cast<Direction>(getValue(FREQSHIFTER_LEFTDIR,
         static_cast<int>(DIR_MAX_ENUM))); if (dir == DIR_NONE) alEffecti(effect,
         AL_FREQUENCY_SHIFTER_LEFT_DIRECTION, AL_FREQUENCY_SHIFTER_DIRECTION_OFF); else if(dir ==
         DIR_UP) alEffecti(effect, AL_FREQUENCY_SHIFTER_LEFT_DIRECTION,
         AL_FREQUENCY_SHIFTER_DIRECTION_UP); else if(dir == DIR_DOWN) alEffecti(effect,
         AL_FREQUENCY_SHIFTER_LEFT_DIRECTION, AL_FREQUENCY_SHIFTER_DIRECTION_DOWN); else
                              alEffecti(effect, AL_FREQUENCY_SHIFTER_LEFT_DIRECTION,
         AL_FREQUENCY_SHIFTER_DEFAULT_LEFT_DIRECTION);

                      dir = static_cast<Direction>(getValue(FREQSHIFTER_RIGHTDIR,
         static_cast<int>(DIR_MAX_ENUM))); if (dir == DIR_NONE) alEffecti(effect,
         AL_FREQUENCY_SHIFTER_RIGHT_DIRECTION, AL_FREQUENCY_SHIFTER_DIRECTION_OFF); else if(dir ==
         DIR_UP) alEffecti(effect, AL_FREQUENCY_SHIFTER_RIGHT_DIRECTION,
         AL_FREQUENCY_SHIFTER_DIRECTION_UP); else if(dir == DIR_DOWN) alEffecti(effect,
         AL_FREQUENCY_SHIFTER_RIGHT_DIRECTION, AL_FREQUENCY_SHIFTER_DIRECTION_DOWN); else
                              alEffecti(effect, AL_FREQUENCY_SHIFTER_RIGHT_DIRECTION,
         AL_FREQUENCY_SHIFTER_DEFAULT_RIGHT_DIRECTION); break;
              }
              case TYPE_MORPHER:
              {
                      Waveform wave = static_cast<Waveform>(getValue(MORPHER_WAVEFORM,
         static_cast<int>(WAVE_MAX_ENUM))); if (wave == WAVE_SINE) alEffecti(effect,
         AL_VOCAL_MORPHER_WAVEFORM, AL_VOCAL_MORPHER_WAVEFORM_SINUSOID); else if (wave ==
         WAVE_TRIANGLE) alEffecti(effect, AL_VOCAL_MORPHER_WAVEFORM,
         AL_VOCAL_MORPHER_WAVEFORM_TRIANGLE); else if (wave == WAVE_SAWTOOTH) alEffecti(effect,
         AL_VOCAL_MORPHER_WAVEFORM, AL_VOCAL_MORPHER_WAVEFORM_SAWTOOTH); else alEffecti(effect,
         AL_VOCAL_MORPHER_WAVEFORM, AL_VOCAL_MORPHER_DEFAULT_WAVEFORM);

                      Phoneme phoneme = static_cast<Phoneme>(getValue(MORPHER_PHONEMEA,
         static_cast<int>(PHONEME_MAX_ENUM))); if (phoneme == PHONEME_MAX_ENUM) alEffecti(effect,
         AL_VOCAL_MORPHER_PHONEMEA, AL_VOCAL_MORPHER_DEFAULT_PHONEMEA); else alEffecti(effect,
         AL_VOCAL_MORPHER_PHONEMEA, phonemeMap[phoneme]);

                      phoneme = static_cast<Phoneme>(getValue(MORPHER_PHONEMEB,
         static_cast<int>(PHONEME_MAX_ENUM))); if (phoneme == PHONEME_MAX_ENUM) alEffecti(effect,
         AL_VOCAL_MORPHER_PHONEMEB, AL_VOCAL_MORPHER_DEFAULT_PHONEMEB); else alEffecti(effect,
         AL_VOCAL_MORPHER_PHONEMEB, phonemeMap[phoneme]);
              ...
              case TYPE_PITCHSHIFTER:
              {
                      float tune = getValue(PITCHSHIFTER_PITCH,
         (float)AL_PITCH_SHIFTER_DEFAULT_COARSE_TUNE + (float)(AL_PITCH_SHIFTER_DEFAULT_FINE_TUNE -
         50) / 100.0 );
    ...
    {
      Waveform wave =
          static_cast<Waveform>(getValue(MODULATOR_WAVEFORM, static_cast<int>(WAVE_MAX_ENUM)));
      if (wave == WAVE_SINE)
	...
	alEffecti(effect, AL_RING_MODULATOR_WAVEFORM, AL_RING_MODULATOR_DEFAULT_WAVEFORM);
      ...
      */
    case TYPE_COMPRESSOR:
      alEffecti(effect, AL_COMPRESSOR_ONOFF,
                getValue(COMPRESSOR_ENABLE, static_cast<int>(AL_COMPRESSOR_DEFAULT_ONOFF)));
      break;
...
const std::map<Effect::Parameter, float> &Effect::getParams() const { return params; }
