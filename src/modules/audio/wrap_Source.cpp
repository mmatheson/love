...
Source *luax_checksource(lua_State *L, int idx) { return luax_checktype<Source>(L, idx); }
...
int w_Source_setPitch(lua_State *L)
{
  ...
  if (p > std::numeric_limits<lua_Number>::max() || p <= 0.0f)
...
int setFilterReadFilter(lua_State *L, int idx, std::map<Filter::Parameter, float> &params)
{
  ...
  while (lua_next(L, idx))
  {
    ...
    if (Filter::getConstant(keystr, param, type) ||
        Filter::getConstant(keystr, param, Filter::TYPE_BASIC))
    {
...
int w_Source_queue(lua_State *L)
{
  ...
  if (luax_istype(L, 2, love::sound::SoundData::type))
  {
    ...
    luax_catchexcept(L,
                     ...
                     {
                       success =
                           t->queue((unsigned char *) s->getData() + offset, length,
  ...
  else if (lua_islightuserdata(L, 2))
  {
    ...
    luax_catchexcept(L,
                     ...
                     {
                       success = t->queue(
                           (void *) ((uintptr_t) lua_touserdata(L, 2) + (uintptr_t) offset), length,
...
{
  luax_markdeprecated(L, "Source:getChannels", API_METHOD, DEPRECATED_RENAMED,
