...
int w_newSource(lua_State *L)
{
  ...
  if (!luax_istype(L, 1, love::sound::SoundData::type) &&
      !luax_istype(L, 1, love::sound::Decoder::type))
  {
    ...
    if (stype == Source::TYPE_QUEUE)
      return luaL_error(
          L, "Cannot create queueable sources using newSource. Use newQueueableSource instead.");
  }

  if (lua_isstring(L, 1) || luax_istype(L, 1, love::filesystem::File::type) ||
      luax_istype(L, 1, love::filesystem::FileData::type))
...
int w_newQueueableSource(lua_State *L)
{
  ...
  luax_catchexcept(L,
                   [&]()
                   {
                     t = instance()->newSource(
                         (int) luaL_checkinteger(L, 1), (int) luaL_checkinteger(L, 2),
                         (int) luaL_checkinteger(L, 3), (int) luaL_optinteger(L, 4, 0));
                   });
...
static std::vector<Source *> readSourceVararg(lua_State *L, int i)
{
  ...
  for (int pos = 0; i <= top; i++, pos++) sources[pos] = luax_checksource(L, i);
...
int w_setEffect(lua_State *L)
{
  ...
  while (lua_next(L, 2))
  {
    ...
    if (Effect::getConstant(keystr, param, type) ||
        Effect::getConstant(keystr, param, Effect::TYPE_BASIC))
    {
#define luax_effecterror(l, t)                                                           \
  luaL_error(l, "Bad parameter type for %s %s: " t " expected, got %s", typestr, keystr, \
             lua_typename(L, -1))
...
int w_getEffect(lua_State *L)
{
  ...
  for (auto p : params)
  {
	...
	/*
	                case Effect::PARAM_DIRECTION:
	                        Effect::getConstant(static_cast<Effect::Direction>((int)p.second),
	   valstr); lua_pushstring(L, valstr); break; case Effect::PARAM_PHONEME:
	                        Effect::getConstant(static_cast<Effect::Phoneme>((int)p.second),
	   valstr); lua_pushstring(L, valstr); break;
	*/
      case Effect::PARAM_TYPE:
...
{
  luax_markdeprecated(L, "love.audio.getSourceCount", API_FUNCTION, DEPRECATED_RENAMED,
                      "love.audio.getActiveSourceCount");
  return w_getActiveSourceCount(L);
}

// List of functions to wrap.
static const luaL_Reg functions[] = {{"getActiveSourceCount", w_getActiveSourceCount},
...
static const lua_CFunction types[] = {luaopen_source, luaopen_recordingdevice, 0};
