...
const char *Mouse::getName() const { return "love.mouse.sdl"; }
...
Mouse::~Mouse()
{
  ...
  for (auto &c : systemCursors) c.second->release();
...
bool Mouse::isCursorSupported() const { return SDL_GetDefaultCursor() != nullptr; }
...
void Mouse::setX(double x) { setPosition(x, getY()); }

void Mouse::setY(double y) { setPosition(getX(), y); }

void Mouse::setVisible(bool visible) { SDL_ShowCursor(visible ? SDL_ENABLE : SDL_DISABLE); }
...
bool Mouse::isVisible() const { return SDL_ShowCursor(SDL_QUERY) == SDL_ENABLE; }
...
bool Mouse::getRelativeMode() const { return SDL_GetRelativeMouseMode() != SDL_FALSE; }
