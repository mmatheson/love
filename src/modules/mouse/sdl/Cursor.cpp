      ...
      systemType(CURSOR_MAX_ENUM)
{
  ...
  SDL_Surface *surface =
      SDL_CreateRGBSurfaceFrom(data->getData(), w, h, 32, pitch, rmask, gmask, bmask, amask);
  if (!surface)
...
void *Cursor::getHandle() const { return cursor; }

Cursor::CursorType Cursor::getType() const { return type; }

Cursor::SystemCursor Cursor::getSystemType() const { return systemType; }

EnumMap<Cursor::SystemCursor, SDL_SystemCursor, Cursor::CURSOR_MAX_ENUM>::Entry
...
EnumMap<Cursor::SystemCursor, SDL_SystemCursor, Cursor::CURSOR_MAX_ENUM> Cursor::systemCursors(
