...
const std::string &TheoraVideoStream::getFilename() const { return demuxer.getFilename(); }
...
const void *TheoraVideoStream::getFrontBuffer() const { return frontBuffer; }

size_t TheoraVideoStream::getSize() const { return sizeof(Frame); }

bool TheoraVideoStream::isPlaying() const { return frameSync->isPlaying() && !demuxer.isEos(); }
...
{
  bool success = demuxer.seek(
      packet, target, [this](int64 granulepos) { return th_granule_time(decoder, granulepos); });
