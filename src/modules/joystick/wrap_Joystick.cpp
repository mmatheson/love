...
Joystick *luax_checkjoystick(lua_State *L, int idx) { return luax_checktype<Joystick>(L, idx); }
...
int w_Joystick_getAxes(lua_State *L)
{
  ...
  for (float value : axes) lua_pushnumber(L, value);
...
int w_Joystick_isDown(lua_State *L)
{
  ...
  {
    for (int i = 0; i < num; i++) buttons.push_back((int) luaL_checkinteger(L, i + 2) - 1);
  }
