...
static inline uint16 normToUint16(double n) { return (uint16) (n * LOVE_UINT16_MAX); }
...
void Font::createTexture()
{
...

  {
    ...
    {
      for (size_t i = 0; i < pixelcount; i++) emptydata[i * 2 + 0] = 255;
    }
  ...
  if (recreatetexture)
  {
    ...
    for (const auto &glyphpair : glyphs) glyphstoadd.push_back(glyphpair.first);

    glyphs.clear();

    for (uint32 g : glyphstoadd) addGlyph(g);
  }
}
...
const Font::Glyph &Font::addGlyph(uint32 glyph)
{
  ...
  StrongRef<love::font::GlyphData> gd(getRasterizerGlyphData(glyph, glyphdpiscale),
                                      Acquire::NORETAIN);
  ...
  if (w > 0 && h > 0)
  {
    ...
    const GlyphVertex verts[4] = {
        {float(-o), float(-o), normToUint16((tX - o) / tWidth), normToUint16((tY - o) / tHeight),
         c},
        {float(-o), (h + o) / glyphdpiscale, normToUint16((tX - o) / tWidth),
         normToUint16((tY + h + o) / tHeight), c},
        {(w + o) / glyphdpiscale, float(-o), normToUint16((tX + w + o) / tWidth),
         normToUint16((tY - o) / tHeight), c},
        {(w + o) / glyphdpiscale, (h + o) / glyphdpiscale, normToUint16((tX + w + o) / tWidth),
         normToUint16((tY + h + o) / tHeight), c}};
...
void Font::getCodepointsFromString(const std::vector<ColoredString> &strs,
...
float Font::getHeight() const { return (float) floorf(height / dpiScale + 0.5f); }
                                                      ...
                                                      TextInfo *info)
{
  ...
  if (info != nullptr)
  {
    ...
    info->height =
        (int) dy + (dx > 0.0f ? floorf(getHeight() * getLineHeight() + 0.5f) : 0) - offset.y;
  }
                                                               ...
                                                               TextInfo *info)
{
  ...
  for (int i = 0; i < (int) lines.size(); i++)
  {
    ...
    std::vector<DrawCommand> newcommands =
        generateVertices(line, constantcolor, vertices, extraspacing, offset);
...
void Font::printv(graphics::Graphics *gfx, const Matrix4 &t,
...
void Font::print(graphics::Graphics *gfx, const std::vector<ColoredString> &text, const Matrix4 &m,
                 const Colorf &constantcolor)
{
  ColoredCodepoints codepoints;
...
void Font::printf(graphics::Graphics *gfx, const std::vector<ColoredString> &text, float wrap,
                  AlignMode align, const Matrix4 &m, const Colorf &constantcolor)
{
  ColoredCodepoints codepoints;
  getCodepointsFromString(text, codepoints);
  ...
  std::vector<GlyphVertex> vertices;
  std::vector<DrawCommand> drawcommands =
      generateVerticesFormatted(codepoints, constantcolor, wrap, align, vertices);
                   ...
                   std::vector<ColoredCodepoints> &lines, std::vector<int> *linewidths)
{
  ...
  while (i < (int) codepoints.cps.size())
  {
	...
	while (!wline.cps.empty() && wline.cps.back() != ' ') wline.cps.pop_back();
...
const Texture::Filter &Font::getFilter() const { return filter; }

int Font::getAscent() const { return floorf(rasterizers[0]->getAscent() / dpiScale + 0.5f); }

int Font::getDescent() const { return floorf(rasterizers[0]->getDescent() / dpiScale + 0.5f); }
...
void Font::setFallbacks(const std::vector<Font *> &fallbacks)
{
  ...
  for (const Font *f : fallbacks) rasterizers.push_back(f->rasterizers[0]);
}

float Font::getDPIScale() const { return dpiScale; }

uint32 Font::getTextureCacheID() const { return textureCacheID; }

bool Font::getConstant(const char *in, AlignMode &out) { return alignModes.find(in, out); }

bool Font::getConstant(AlignMode in, const char *&out) { return alignModes.find(in, out); }

std::vector<std::string> Font::getConstants(AlignMode) { return alignModes.getNames(); }
