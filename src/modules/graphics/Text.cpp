...
Text::~Text() { delete vertex_buffer; }
...
void Text::regenerateVertices()
{
  ...
  if (font->getTextureCacheID() != texture_cache_id)
  {
    ...
    for (const TextData &t : textdata) addTextData(t);
...
void Text::addTextData(const TextData &t)
{
  ...
  if (t.align == Font::ALIGN_MAX_ENUM)
    new_commands = font->generateVertices(t.codepoints, constantcolor, vertices, 0.0f,
                                          Vector2(0.0f, 0.0f), &text_info);
  else
    new_commands = font->generateVerticesFormatted(t.codepoints, constantcolor, t.wrap, t.align,
                                                   vertices, &text_info);
  ...
  if (!new_commands.empty())
  {
    ...
    for (Font::DrawCommand &cmd : new_commands) cmd.startvertex += (int) voffset;
...
Font *Text::getFont() const { return font.get(); }
...
void Text::draw(Graphics *gfx, const Matrix4 &m)
{
  ...
  for (const Font::DrawCommand &cmd : draw_commands)
    gfx->drawQuads(cmd.startvertex / 4, cmd.vertexcount / 4, vertexAttributes, vertexBuffers,
                   cmd.texture);
}
