...
int w_ParticleSystem_setEmissionArea(lua_State *L)
{
  ...
  if (str && !ParticleSystem::getConstant(str, distribution))
    return luax_enumerror(L, "particle distribution", ParticleSystem::getConstants(distribution),
                          str);
...
int w_ParticleSystem_getEmissionArea(lua_State *L)
{
  ...
  ParticleSystem::AreaSpreadDistribution distribution =
      t->getEmissionArea(p, angle, directionRelativeToCenter);
  const char *str;
...
int w_ParticleSystem_setSizes(lua_State *L)
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < nSizes; ++i) sizes[i] = luax_checkfloat(L, (int) (1 + i + 1));
...
int w_ParticleSystem_getSizes(lua_State *L)
{
  ...
  for (size_t i = 0; i < sizes.size(); i++) lua_pushnumber(L, sizes[i]);
...
int w_ParticleSystem_setColors(lua_State *L)
{
  ...
  else  // setColors(r,g,b,a, r,g,b,a, ...)
  {
    ...
    if (cargs != 3 && (cargs % 4 != 0 || cargs == 0))
      return luaL_error(L, "Expected red, green, blue, and alpha. Only got %d of 4 components.",
                        cargs % 4);
...
{
  luax_markdeprecated(L, "ParticleSystem:setAreaSpread", API_METHOD, DEPRECATED_REPLACED,
...
{
  luax_markdeprecated(L, "ParticleSystem:getAreaSpread", API_METHOD, DEPRECATED_REPLACED,
