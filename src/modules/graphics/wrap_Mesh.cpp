...
Mesh *luax_checkmesh(lua_State *L, int idx) { return luax_checktype<Mesh>(L, idx); }
...
static inline size_t readUnorm8Data(lua_State *L, int components, const char *data)
{
  ...
  for (int i = 0; i < components; i++) lua_pushnumber(L, (lua_Number) componentdata[i] / 255.0);
...
static inline size_t readUnorm16Data(lua_State *L, int components, const char *data)
{
  ...
  for (int i = 0; i < components; i++) lua_pushnumber(L, (lua_Number) componentdata[i] / 65535.0);
...
static inline size_t readFloatData(lua_State *L, int components, const char *data)
{
  ...
  for (int i = 0; i < components; i++) lua_pushnumber(L, componentdata[i]);
...
int w_Mesh_setVertices(lua_State *L)
{
  ...
  if (luax_istype(L, 2, Data::type))
  {
    ...
    if (vertstart + vertcount > totalverts)
      return luaL_error(L, "Too many vertices (expected at most %d, got %d)",
                        totalverts - vertstart, vertcount);
  ...
  if (vertstart + vertcount > totalverts)
    return luaL_error(L, "Too many vertices (expected at most %d, got %d)", totalverts - vertstart,
                      vertcount);

  const std::vector<Mesh::AttribFormat> &vertexformat = t->getVertexFormat();

  int ncomponents = 0;
  for (const Mesh::AttribFormat &format : vertexformat) ncomponents += format.components;
  ...
  for (int i = 0; i < vertcount; i++)
  {
    ...
    for (int j = 1; j <= ncomponents; j++) lua_rawgeti(L, -j, j);
...
int w_Mesh_setVertex(lua_State *L)
{
  ...
  if (istable)
  {
    for (const Mesh::AttribFormat &format : vertexformat)
    {
      for (int i = idx; i < idx + format.components; i++) lua_rawgeti(L, 3, i);

      // Fetch the values from Lua and store them in data buffer.
      writtendata = luax_writeAttributeData(L, -format.components, format.type, format.components,
                                            writtendata);
...
int w_Mesh_setVertexAttribute(lua_State *L)
{
  ...
  luax_catchexcept(
      L, [&]() { t->setVertexAttribute(vertindex, attribindex, data, sizeof(float) * 4); });
  return 0;
}
...
int w_Mesh_getVertexAttribute(lua_State *L)
{
  ...
  luax_catchexcept(
      L, [&]() { t->getVertexAttribute(vertindex, attribindex, data, sizeof(float) * 4); });
...
int w_Mesh_getVertexFormat(lua_State *L)
{
  ...
  for (size_t i = 0; i < vertexformat.size(); i++)
  {
    if (!vertex::getConstant(vertexformat[i].type, tname))
      return luax_enumerror(L, "vertex attribute data type",
                            vertex::getConstants(vertexformat[i].type), tname);
...
int w_Mesh_setVertexMap(lua_State *L)
{
  ...
  if (luax_istype(L, 2, Data::type))
  {
    ...
    luax_catchexcept(
        L, [&]() { t->setVertexMap(indextype, d->getData(), indexcount * datatypesize); });
    return 0;
  }
  ...
  {
    for (int i = 0; i < nargs; i++) vertexmap.push_back(uint32(luaL_checkinteger(L, i + 2) - 1));
  }
...
static const luaL_Reg w_Mesh_functions[] = {{"setVertices", w_Mesh_setVertices},
