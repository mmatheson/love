...
void Shader::checkMainTextureType(TextureType textype, bool isDepthSampler) const
{
  ...
  if (info->textureType != TEXTURE_MAX_ENUM && info->textureType != textype)
  {
    ...
    throw love::Exception(
        "Texture's type (%s) must match the type of the shader's main texture type (%s).",
        textypestr, shadertextypestr);
  }
  ...
  if (info->isDepthSampler != isDepthSampler)
  {
    if (info->isDepthSampler)
      throw love::Exception(
          "Depth comparison samplers in shaders can only be used with depth textures which have "
          "depth comparison set.");
    else
      throw love::Exception(
          "Depth textures which have depth comparison set can only be used with depth/shadow "
          "samplers in shaders.");
  }
}
...
bool Shader::initialize() { return glslang::InitializeProcess(); }

void Shader::deinitialize() { glslang::FinalizeProcess(); }

bool Shader::getConstant(const char *in, Language &out) { return languages.find(in, out); }

bool Shader::getConstant(Language in, const char *&out) { return languages.find(in, out); }

bool Shader::getConstant(const char *in, BuiltinUniform &out) { return builtinNames.find(in, out); }

bool Shader::getConstant(BuiltinUniform in, const char *&out) { return builtinNames.find(in, out); }
...
StringMap<Shader::Language, Shader::LANGUAGE_MAX_ENUM> Shader::languages(
    Shader::languageEntries, sizeof(Shader::languageEntries));
...
StringMap<Shader::BuiltinUniform, Shader::BUILTIN_MAX_ENUM> Shader::builtinNames(
    Shader::builtinNameEntries, sizeof(Shader::builtinNameEntries));
