...
class Graphics : public Module
{
 public:
  ...
  struct ColorMask
  {
    ...
    bool operator!=(const ColorMask &m) const { return !(operator==(m)); }
  };
  ...
  struct DrawIndexedCommand
  {
    ...
    DrawIndexedCommand(const vertex::Attributes *attribs, const vertex::BufferBindings *buffers,
                       Resource *indexbuffer)
        : attributes(attribs),
          buffers(buffers),
  ...
  class TempTransform
  {
   public:
    ...
    ~TempTransform() { gfx->popTransform(); }
  ...
  typedef void (*ScreenshotCallback)(const ScreenshotInfo *info, love::image::ImageData *i,
                                     void *ud);
  ...
  struct RenderTargets
  {
    ...
    const RenderTarget &getFirstTarget() const { return colors.empty() ? depthStencil : colors[0]; }
  ...
  virtual Image *newImage(TextureType textype, PixelFormat format, int width, int height,
                          int slices, const Image::Settings &settings) = 0;
  ...
  virtual Buffer *newBuffer(size_t size, const void *data, BufferType type, vertex::Usage usage,
                            uint32 mapflags) = 0;
  ...
  Mesh *newMesh(const std::vector<Mesh::AttribFormat> &vertexformat, int vertexcount,
                PrimitiveType drawmode, vertex::Usage usage);
  Mesh *newMesh(const std::vector<Mesh::AttribFormat> &vertexformat, const void *data,
                size_t datasize, PrimitiveType drawmode, vertex::Usage usage);

  Text *newText(Font *font, const std::vector<Font::ColoredString> &text = {});

  bool validateShader(bool gles, const std::string &vertex, const std::string &pixel,
                      std::string &err);
  ...
  virtual void clear(const std::vector<OptionalColorf> &colors, OptionalInt stencil,
   ...
   **/
  virtual bool setMode(int width, int height, int pixelwidth, int pixelheight,
                       bool windowhasstencil) = 0;
   ...
   **/
  void printf(const std::vector<Font::ColoredString> &str, float wrap, Font::AlignMode align,
              const Matrix4 &m);
  void printf(const std::vector<Font::ColoredString> &str, Font *font, float wrap,
  ...
  virtual void drawQuads(int start, int count, const vertex::Attributes &attributes,
                         const vertex::BufferBindings &buffers, Texture *texture) = 0;
  ...
  static DefaultShaderCode defaultShaderCode[Shader::STANDARD_MAX_ENUM][Shader::LANGUAGE_MAX_ENUM]
                                            [2];
 ...
 protected:
  ...
  virtual ShaderStage *newShaderStageInternal(ShaderStage::StageType stage,
                                              const std::string &cachekey,
                                              const std::string &source, bool gles) = 0;
  ...
  virtual void setCanvasInternal(const RenderTargets &rts, int w, int h, int pixelw, int pixelh,
                                 bool hasSRGBcanvas) = 0;
