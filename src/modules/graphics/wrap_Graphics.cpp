...
int w_clear(lua_State *L)
{
  ...
  if (argtype == LUA_TTABLE)
  {
	...
	continue;
      ...
      for (int j = 1; j <= 4; j++) lua_rawgeti(L, i + 1, j);
...
int w_getCanvas(lua_State *L)
{
  ...
  {
    for (const auto &rt : targets.colors) luax_pushtype(L, rt.canvas);

    return ntargets;
  }
}

static void screenshotFunctionCallback(const Graphics::ScreenshotInfo *info,
                                       love::image::ImageData *i, void *gd)
{
  if (info == nullptr)
    return;
...
static void screenshotFileCallback(const Graphics::ScreenshotInfo *info, love::image::ImageData *i,
                                   void * /*gd*/)
{
  if (info == nullptr)
    return;
...
static void screenshotChannelCallback(const Graphics::ScreenshotInfo *info,
                                      love::image::ImageData *i, void * /*gd*/)
{
  if (info == nullptr)
    return;
...
int w_captureScreenshot(lua_State *L)
{
  ...
  else if (lua_isstring(L, 1))
  {
    ...
    if (!image::ImageData::getConstant(ext.c_str(), format))
      return luax_enumerror(L, "encoded image format", image::ImageData::getConstants(format),
                            ext.c_str());
...
int w_setScissor(lua_State *L)
{
  ...
  if (nargs == 0 ||
      (nargs == 4 && lua_isnil(L, 1) && lua_isnil(L, 2) && lua_isnil(L, 3) && lua_isnil(L, 4)))
  {
    instance()->setScissor();
    return 0;
  }
...
int w_stencil(lua_State *L)
{
  ...
  if (argtype == LUA_TNONE || argtype == LUA_TNIL ||
      (argtype == LUA_TBOOLEAN && luax_toboolean(L, 4) == false))
    stencilclear.set(0);
  else if (argtype == LUA_TNUMBER)
...
static std::pair<StrongRef<image::ImageData>, StrongRef<image::CompressedImageData>> getImageData(
    lua_State *L, int idx, bool allowcompressed, float *dpiscale)
{
  ...
  else if (filesystem::luax_cangetdata(L, idx))
  {
    ...
    if (allowcompressed && imagemodule->isCompressed(fdata))
      luax_catchexcept(
          L, [&]() { cdata.set(imagemodule->newCompressedData(fdata), Acquire::NORETAIN); });
    else
      luax_catchexcept(L,
                       [&]() { idata.set(imagemodule->newImageData(fdata), Acquire::NORETAIN); });
  }
  else
    idata.set(image::luax_checkimagedata(L, idx));
...
static int w__pushNewImage(lua_State *L, Image::Slices &slices, const Image::Settings &settings)
{
  StrongRef<Image> i;
  luax_catchexcept(
      L, [&]() { i.set(instance()->newImage(slices, settings), Acquire::NORETAIN); },
      [&](bool) { slices.clear(); });
...
int w_newCubeImage(lua_State *L)
{
  ...
  if (!lua_istable(L, 1))
  {
    ...
    if (data.first.get())
    {
      ...
      for (int i = 0; i < (int) faces.size(); i++) slices.set(i, 0, faces[i]);
    }
    else
  ...
  else
  {
	...
	if (data.first.get())
	{
	  if (usemipmaps || data.first->getWidth() != data.first->getHeight())
	  {
	    ...
	    for (int face = 0; face < (int) faces.size(); face++) slices.set(face, i, faces[i]);
	  }
	  else
...
int w_newVolumeImage(lua_State *L)
{
  ...
  else
  {
    ...
    if (data.first.get())
    {
      ...
      for (int i = 0; i < (int) layers.size(); i++) slices.set(i, 0, layers[i]);
    }
    else
      slices.add(data.second, 0, 0, true, settings.mipmaps);
  }
...
int w_newFont(lua_State *L)
{
  ...
  {
    std::vector<int> idxs;
    for (int i = 0; i < lua_gettop(L); i++) idxs.push_back(i + 1);
...
int w_newSpriteBatch(lua_State *L)
{
  ...
  luax_catchexcept(L, [&]() { t = instance()->newSpriteBatch(texture, size, usage); });
...
int w_newParticleSystem(lua_State *L)
{
  ...
  luax_catchexcept(L, [&]() { t = instance()->newParticleSystem(texture, int(size)); });
...
int w_newCanvas(lua_State *L)
{
  ...
  {
    luax_checktablefields<Canvas::SettingType>(L, startidx, "canvas setting name",
                                               Canvas::getConstant);

    settings.dpiScale = (float) luax_numberflag(
        L, startidx, Canvas::getConstant(Canvas::SETTING_DPI_SCALE), settings.dpiScale);
    settings.msaa =
        luax_intflag(L, startidx, Canvas::getConstant(Canvas::SETTING_MSAA), settings.msaa);
...
static int w_getShaderSource(lua_State *L, int startidx, bool gles, std::string &vertexsource,
...
static Mesh *newStandardMesh(lua_State *L)
{
  ...
  if (lua_istable(L, 1))
  {
	...
	return nullptr;
      ...
      for (int j = 1; j <= 8; j++) lua_rawgeti(L, -j, j);
...
static Mesh *newCustomMesh(lua_State *L)
{
  ...
  for (int i = 1; i <= (int) luax_objlen(L, 1); i++)
  {
    ...
    for (int j = 1; j <= 3; j++) lua_rawgeti(L, -j, j);
    ...
    {
      luaL_error(L, "Number of vertex attribute components must be between 1 and 4 (got %d)",
                 format.components);
      return nullptr;
    }
  ...
  if (lua_isnumber(L, 2))
  {
    ...
    luax_catchexcept(
        L, [&]() { t = instance()->newMesh(vertexformat, vertexcount, drawmode, usage); });
  }
  else if (luax_istype(L, 2, Data::type))
  {
    ...
    luax_catchexcept(L,
                     [&]() {
                       t = instance()->newMesh(vertexformat, data->getData(), data->getSize(),
                                               drawmode, usage);
                     });
  }
  else
  {
    ...
    for (const Mesh::AttribFormat &format : vertexformat) vertexcomponents += format.components;

    size_t numvertices = luax_objlen(L, 2);

    luax_catchexcept(
        L, [&]() { t = instance()->newMesh(vertexformat, numvertices, drawmode, usage); });
	...
	luax_catchexcept(
	    L, [&]() { t->setVertexAttribute(vertindex, i, data, sizeof(float) * 4); },
	    [&](bool diderror)
	    {
	      if (diderror)
	        t->release();
...
int w_newMesh(lua_State *L)
{
  ...
  if (arg1type == LUA_TTABLE &&
      (arg2type == LUA_TTABLE || arg2type == LUA_TNUMBER || arg2type == LUA_TUSERDATA))
    t = newCustomMesh(L);
  else
    t = newStandardMesh(L);
...
int w_setColor(lua_State *L)
{
  ...
  if (lua_istable(L, 1))
  {
    for (int i = 1; i <= 4; i++) lua_rawgeti(L, 1, i);
...
int w_setDefaultShaderCode(lua_State *L)
{
  for (int i = 0; i < 2; i++)
  {
	...
	continue;
      ...
      Graphics::defaultShaderCode[Shader::STANDARD_DEFAULT][lang][i]
          .source[ShaderStage::STAGE_VERTEX] = vertex;
      Graphics::defaultShaderCode[Shader::STANDARD_DEFAULT][lang][i]
          .source[ShaderStage::STAGE_PIXEL] = pixel;

      Graphics::defaultShaderCode[Shader::STANDARD_VIDEO][lang][i]
          .source[ShaderStage::STAGE_VERTEX] = vertex;
      Graphics::defaultShaderCode[Shader::STANDARD_VIDEO][lang][i]
          .source[ShaderStage::STAGE_PIXEL] = videopixel;

      Graphics::defaultShaderCode[Shader::STANDARD_ARRAY][lang][i]
          .source[ShaderStage::STAGE_VERTEX] = vertex;
      Graphics::defaultShaderCode[Shader::STANDARD_ARRAY][lang][i]
          .source[ShaderStage::STAGE_PIXEL] = arraypixel;
    }
  }
...
static int w__getFormats(lua_State *L, int idx, bool (*isFormatSupported)(PixelFormat),
                         bool (*ignore)(PixelFormat))
{
...
int w_draw(lua_State *L)
{
  ...
  luax_checkstandardtransform(L, startidx,
                              [&](const Matrix4 &m)
                              {
                                luax_catchexcept(L,
                                                 [&]()
...
int w_print(lua_State *L)
{
  ...
  if (luax_istype(L, 2, Font::type))
  {
    ...
    luax_checkstandardtransform(
        L, 3,
        [&](const Matrix4 &m) { luax_catchexcept(L, [&]() { instance()->print(str, font, m); }); });
  }
  else
  {
    luax_checkstandardtransform(
        L, 2, [&](const Matrix4 &m) { luax_catchexcept(L, [&]() { instance()->print(str, m); }); });
  }
...
int w_points(lua_State *L)
{
  ...
  if (is_table)
  {
    if (is_table_of_tables)
    {
      ...
      {
	lua_rawgeti(L, 1, i + 1);
	for (int j = 1; j <= 6; j++) lua_rawgeti(L, -j, j);
...
int w_line(lua_State *L)
{
  ...
  luax_catchexcept(L, [&]() { instance()->polyline(coords, numvertices); });
...
static const luaL_Reg functions[] = {{"reset", w_reset},
                                     {"clear", w_clear},
                                     {"discard", w_discard},
                                     {"present", w_present},
...
static const lua_CFunction types[] = {luaopen_drawable,
                                      luaopen_texture,
                                      luaopen_font,
                                      luaopen_image,
                                      luaopen_quad,
...
extern "C" int luaopen_love_graphics(lua_State *L)
{
  ...
  if (luaL_loadbuffer(L, (const char *) graphics_lua, sizeof(graphics_lua),
                      "=[love \"wrap_Graphics.lua\"]") == 0)
    lua_call(L, 0, 0);
  else
    lua_error(L);

  if (luaL_loadbuffer(L, (const char *) graphics_shader_lua, sizeof(graphics_shader_lua),
                      "=[love \"wrap_GraphicsShader.lua\"]") == 0)
    lua_call(L, 0, 0);
  else
    lua_error(L);
