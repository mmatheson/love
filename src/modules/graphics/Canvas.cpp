    ...
    : Texture(settings.type)
{
  ...
  if (!gfx->isCanvasFormatSupported(format, readable))
  {
    ...
    throw love::Exception("The %s%s canvas format is not supported by your graphics drivers.", fstr,
                          readablestr);
  }
...
Canvas::~Canvas() { canvasCount--; }

Canvas::MipmapMode Canvas::getMipmapMode() const { return settings.mipmaps; }

int Canvas::getRequestedMSAA() const { return settings.msaa; }

love::image::ImageData *Canvas::newImageData(love::image::Image *module, int slice, int mipmap,
                                             const Rect &r)
{
  if (!isReadable())
  ...
  if (r.x < 0 || r.y < 0 || r.w <= 0 || r.h <= 0 || (r.x + r.w) > getPixelWidth(mipmap) ||
      (r.y + r.h) > getPixelHeight(mipmap))
    throw love::Exception("Invalid rectangle dimensions.");

  if (slice < 0 || (texType == TEXTURE_VOLUME && slice >= getDepth(mipmap)) ||
      (texType == TEXTURE_2D_ARRAY && slice >= layers) || (texType == TEXTURE_CUBE && slice >= 6))
  {
...
bool Canvas::getConstant(const char *in, MipmapMode &out) { return mipmapModes.find(in, out); }

bool Canvas::getConstant(MipmapMode in, const char *&out) { return mipmapModes.find(in, out); }

std::vector<std::string> Canvas::getConstants(MipmapMode) { return mipmapModes.getNames(); }

bool Canvas::getConstant(const char *in, SettingType &out) { return settingTypes.find(in, out); }

bool Canvas::getConstant(SettingType in, const char *&out) { return settingTypes.find(in, out); }
...
std::vector<std::string> Canvas::getConstants(SettingType) { return settingTypes.getNames(); }
...
StringMap<Canvas::MipmapMode, Canvas::MIPMAPS_MAX_ENUM> Canvas::mipmapModes(
    Canvas::mipmapEntries, sizeof(Canvas::mipmapEntries));
...
StringMap<Canvas::SettingType, Canvas::SETTING_MAX_ENUM>::Entry Canvas::settingTypeEntries[] = {
    ...
    {"mipmaps", SETTING_MIPMAPS},    {"format", SETTING_FORMAT}, {"type", SETTING_TYPE},
    {"dpiscale", SETTING_DPI_SCALE}, {"msaa", SETTING_MSAA},     {"readable", SETTING_READABLE},
};

StringMap<Canvas::SettingType, Canvas::SETTING_MAX_ENUM> Canvas::settingTypes(
    Canvas::settingTypeEntries, sizeof(Canvas::settingTypeEntries));
