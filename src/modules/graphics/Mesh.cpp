...
static_assert(offsetof(Vertex, x) == sizeof(float) * 0,
              "Incorrect position offset in Vertex struct");
static_assert(offsetof(Vertex, s) == sizeof(float) * 2,
              "Incorrect texture coordinate offset in Vertex struct");
static_assert(offsetof(Vertex, color.r) == sizeof(float) * 4,
              "Incorrect color offset in Vertex struct");
...
Mesh::Mesh(graphics::Graphics *gfx, const std::vector<AttribFormat> &vertexformat, const void *data,
           size_t datasize, PrimitiveType drawmode, vertex::Usage usage)
      ...
      rangeCount(-1)
{
  ...
  vertexBuffer = gfx->newBuffer(datasize, data, BUFFER_VERTEX, usage,
                                Buffer::MAP_EXPLICIT_RANGE_MODIFY | Buffer::MAP_READ);

  vertexScratchBuffer = new char[vertexStride];
}

Mesh::Mesh(graphics::Graphics *gfx, const std::vector<AttribFormat> &vertexformat, int vertexcount,
           PrimitiveType drawmode, vertex::Usage usage)
    : vertexFormat(vertexformat),
      vertexBuffer(nullptr),
      ...
      rangeCount(-1)
{
  ...
  vertexBuffer = gfx->newBuffer(buffersize, nullptr, BUFFER_VERTEX, usage,
                                Buffer::MAP_EXPLICIT_RANGE_MODIFY | Buffer::MAP_READ);
  ...
  for (const AttribFormat &format : vertexFormat)
  {
    ...
    if (size % 4 != 0)
      throw love::Exception(
          "Vertex attributes must have enough components to be a multiple of 32 bits.");
...
size_t Mesh::getAttributeOffset(size_t attribindex) const
{
  ...
  for (size_t i = 0; i < attribindex; i++) offset += attributeSizes[i];
...
void *Mesh::getVertexScratchBuffer() { return vertexScratchBuffer; }
...
size_t Mesh::getVertexStride() const { return vertexStride; }

const std::vector<Mesh::AttribFormat> &Mesh::getVertexFormat() const { return vertexFormat; }
...
void Mesh::setAttributeEnabled(const std::string &name, bool enable)
{
  ...
  if (it == attachedAttributes.end())
    throw love::Exception("Mesh does not have an attached vertex attribute named '%s'",
                          name.c_str());
                           ...
                           AttributeStep step)
{
  ...
  if (newattrib.index < 0)
    throw love::Exception("The specified mesh does not have a vertex attribute named '%s'",
                          attachname.c_str());
...
void *Mesh::mapVertexData() { return vertexBuffer->map(); }
 ...
 **/
template <typename T>
static void copyToIndexBuffer(const std::vector<uint32> &indices, Buffer::Mapper &buffermap,
                              size_t maxval)
{
...
void Mesh::setVertexMap(const std::vector<uint32> &map)
{
  ...
  if (!indexBuffer && size > 0)
  {
    ...
    indexBuffer =
        gfx->newBuffer(size, nullptr, BUFFER_INDEX, vertexBuffer->getUsage(), Buffer::MAP_READ);
  }
...
void Mesh::setVertexMap(IndexDataType datatype, const void *data, size_t datasize)
{
  ...
  if (!indexBuffer && datasize > 0)
  {
    auto gfx = Module::getInstance<graphics::Graphics>(Module::M_GRAPHICS);
    indexBuffer =
        gfx->newBuffer(datasize, nullptr, BUFFER_INDEX, vertexBuffer->getUsage(), Buffer::MAP_READ);
  }
...
void Mesh::setVertexMap() { useIndexBuffer = false; }
...
{
  T *elems = (T *) buffer;
  for (size_t i = 0; i < count; i++) indices.push_back((uint32) elems[i]);
}
...
size_t Mesh::getVertexMapCount() const { return indexCount; }

void Mesh::setTexture(Texture *tex) { texture.set(tex); }

void Mesh::setTexture() { texture.set(nullptr); }

Texture *Mesh::getTexture() const { return texture.get(); }

void Mesh::setDrawMode(PrimitiveType mode) { primitiveType = mode; }

PrimitiveType Mesh::getDrawMode() const { return primitiveType; }
...
void Mesh::setDrawRange() { rangeStart = rangeCount = -1; }
...
void Mesh::draw(Graphics *gfx, const love::Matrix4 &m) { drawInstanced(gfx, m, 1); }
