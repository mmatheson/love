...
Shader *luax_checkshader(lua_State *L, int idx) { return luax_checktype<Shader>(L, idx); }
...
static void _updateNumbers(lua_State *L, int startidx, T *values, int components, int count)
{
  if (components == 1)
  {
    for (int i = 0; i < count; ++i) values[i] = (T) checknum(L, startidx + i);
  }
  else
  {
...
int w_Shader_sendFloats(lua_State *L, int startidx, Shader *shader, const Shader::UniformInfo *info,
                        bool colors)
{
  ...
  if (colors)
    _updateNumbers<float, lua_Number, luax_checknumberclamped01>(L, startidx, values, components,
                                                                 count);
  else
...
{
  int count = _getCount(L, startidx, info);
  _updateNumbers<int, lua_Integer, luaL_checkinteger>(L, startidx, info->ints, info->components,
                                                      count);
  luax_catchexcept(L, [&]() { shader->updateUniform(info, count); });
  return 0;
}

int w_Shader_sendUnsignedInts(lua_State *L, int startidx, Shader *shader,
                              const Shader::UniformInfo *info)
{
  int count = _getCount(L, startidx, info);
  _updateNumbers<unsigned int, lua_Integer, luaL_checkinteger>(L, startidx, info->uints,
                                                               info->components, count);
  luax_catchexcept(L, [&]() { shader->updateUniform(info, count); });
  return 0;
}

int w_Shader_sendBooleans(lua_State *L, int startidx, Shader *shader,
                          const Shader::UniformInfo *info)
{
  int count = _getCount(L, startidx, info);
  int components = info->components;
...
int w_Shader_sendMatrices(lua_State *L, int startidx, Shader *shader,
                          const Shader::UniformInfo *info)
{
...
int w_Shader_sendTextures(lua_State *L, int startidx, Shader *shader,
                          const Shader::UniformInfo *info)
{
  int count = _getCount(L, startidx, info);
...
static int w_Shader_sendLuaValues(lua_State *L, int startidx, Shader *shader,
                                  const Shader::UniformInfo *info, const char *name)
{
...
static int w_Shader_sendData(lua_State *L, int startidx, Shader *shader,
                             const Shader::UniformInfo *info, bool colors)
{
  if (info->baseType == Shader::UNIFORM_SAMPLER)
    return luaL_error(
        L, "Uniform sampler values (textures) cannot be sent to Shaders via Data objects.");
  ...
  if (!lua_isnoneornil(L, startidx + 2))
  {
    ...
    if (sizearg <= 0)
      ...
      return luaL_error(L, "Size (%d) must be a multiple of the uniform's size in bytes (%d).",
                        sizearg, uniformstride);
...
int w_Shader_send(lua_State *L)
{
  ...
  if (info == nullptr)
    return luaL_error(L,
                      "Shader uniform '%s' does not exist.\nA common error is to define but not "
                      "use the variable.",
                      name);
  ...
  if (luax_istype(L, startidx, Data::type) ||
      (info->baseType == Shader::UNIFORM_MATRIX && luax_istype(L, startidx + 1, Data::type)))
...
static const luaL_Reg w_Shader_functions[] = {{"getWarnings", w_Shader_getWarnings},
                                              {"send", w_Shader_send},
