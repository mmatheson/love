...
class StreamBufferClientMemory final : public love::graphics::StreamBuffer
{
 public:
  ...
  virtual ~StreamBufferClientMemory() { delete[] data; }

  MapInfo map(size_t /*minsize*/) override { return MapInfo(data, bufferSize); }

  size_t unmap(size_t /*usedsize*/) override { return (size_t) data; }
...
class StreamBufferSubDataOrphan final : public love::graphics::StreamBuffer, public Volatile
{
 public:
  ...
  void markUsed(size_t usedsize) override { frameGPUReadOffset += usedsize; }
...
class StreamBufferMapSync final : public StreamBufferSync, public Volatile
{
 public:
  ...
  ~StreamBufferMapSync() { unloadVolatile(); }
  ...
  void unloadVolatile() override
  {
    ...
    for (FenceSync &sync : syncs) sync.cleanup();
  }
...
class StreamBufferPersistentMapSync final : public StreamBufferSync, public Volatile
{
 public:
  ...
  ~StreamBufferPersistentMapSync() { unloadVolatile(); }
        ...
        alignedSize(0)
  {
    ...
    if (!loadVolatile())
    {
      ...
      throw love::Exception(
          "AMD Pinned Memory StreamBuffer implementation failed to create buffer (address: %p, "
          "alignment: %ld, aiigned size: %ld)",
          pointer, alignment, alignedSize);
    }
  }
