...
void OpenGL::setupContext()
{
  ...
  for (int i = 0; i < 2; i++) state.boundFramebuffers[i] = std::numeric_limits<GLuint>::max();
...
void OpenGL::initVendor()
{
  ...
  if (strstr(vstr, "ATI Technologies") || strstr(vstr, "AMD") ||
      strstr(vstr, "Advanced Micro Devices"))
    vendor = VENDOR_AMD;
...
void OpenGL::initMaxValues()
{
  ...
  baseVertexSupported = GLAD_VERSION_3_2 || GLAD_ES_VERSION_3_2 ||
                        GLAD_ARB_draw_elements_base_vertex || GLAD_OES_draw_elements_base_vertex ||
...
void OpenGL::setVertexAttributes(const vertex::Attributes &attributes,
                                 const vertex::BufferBindings &buffers)
{
  ...
  while (allbits)
  {
	...
	glVertexAttribDivisor(i, divisor);
      ...
      const void *offsetpointer =
          reinterpret_cast<void *>(bufferinfo.offset + attrib.offsetFromVertex);
  ...
  state.instancedAttribArrays =
      instanceattribbits | (state.instancedAttribArrays & (~attributes.enableBits));
...
Rect OpenGL::getViewport() const { return state.viewport; }
...
void OpenGL::setConstantColor(const Colorf &color) { state.constantColor = color; }

const Colorf &OpenGL::getConstantColor() const { return state.constantColor; }
...
float OpenGL::getPointSize() const { return state.pointSize; }
...
void OpenGL::framebufferTexture(GLenum attachment, TextureType texType, GLuint texture, int level,
                                int layer, int face)
{
...
bool OpenGL::hasDepthWrites() const { return state.depthWritesEnabled; }
...
GLuint OpenGL::getDefaultTexture(TextureType type) const { return state.defaultTexture[type]; }
...
void OpenGL::bindTextureToUnit(TextureType target, GLuint texture, int textureunit,
                               bool restoreprev, bool bindforedit)
{
...
bool OpenGL::rawTexStorage(TextureType target, int levels, PixelFormat pixelformat, bool &isSRGB,
                           int width, int height, int depth)
{
  ...
  else
  {
	...
	for (int face = 0; face < faces; face++)
	{
	  ...
	  glTexImage2D(gltarget, level, fmt.internalformat, w, h, 0, fmt.externalformat, fmt.type,
	               nullptr);
	}
      }
      else if (target == TEXTURE_2D_ARRAY || target == TEXTURE_VOLUME)
      {
	if (target == TEXTURE_VOLUME && GLAD_ES_VERSION_2_0 && GLAD_OES_texture_3D &&
	    !GLAD_ES_VERSION_3_0)
	{
	  glTexImage3DOES(gltarget, level, fmt.internalformat, w, h, d, 0, fmt.externalformat,
	                  fmt.type, nullptr);
	}
	else
	{
	  glTexImage3D(gltarget, level, fmt.internalformat, w, h, d, 0, fmt.externalformat,
	               fmt.type, nullptr);
	}
      }
...
bool OpenGL::isPixelShaderHighpSupported() const { return pixelShaderHighpSupported; }

bool OpenGL::isInstancingSupported() const
{
  return GLAD_ES_VERSION_3_0 || GLAD_VERSION_3_3 || GLAD_ARB_instanced_arrays ||
         GLAD_EXT_instanced_arrays || GLAD_ANGLE_instanced_arrays;
}
...
bool OpenGL::isSamplerLODBiasSupported() const { return GLAD_VERSION_1_4; }

bool OpenGL::isBaseVertexSupported() const { return baseVertexSupported; }

int OpenGL::getMax2DTextureSize() const { return std::max(max2DTextureSize, 1); }

int OpenGL::getMax3DTextureSize() const { return std::max(max3DTextureSize, 1); }

int OpenGL::getMaxCubeTextureSize() const { return std::max(maxCubeTextureSize, 1); }

int OpenGL::getMaxTextureLayers() const { return std::max(maxTextureArrayLayers, 1); }
...
int OpenGL::getMaxRenderbufferSamples() const { return maxRenderbufferSamples; }

int OpenGL::getMaxTextureUnits() const { return maxTextureUnits; }

float OpenGL::getMaxPointSize() const { return maxPointSize; }

float OpenGL::getMaxAnisotropy() const { return maxAnisotropy; }

float OpenGL::getMaxLODBias() const { return maxLODBias; }

bool OpenGL::isCoreProfile() const { return coreProfile; }

OpenGL::Vendor OpenGL::getVendor() const { return vendor; }
                                                 ...
                                                 bool &isSRGB)
{
  ...
  switch (pixelformat)
  {
    ...
    case PIXELFORMAT_DEPTH24:
      if (GLAD_ES_VERSION_2_0 && !GLAD_ES_VERSION_3_0 && !GLAD_OES_depth24 &&
          GLAD_OES_packed_depth_stencil)
      {
	f.internalformat = GL_DEPTH24_STENCIL8;
	f.externalformat = GL_DEPTH_STENCIL;
	f.type = GL_UNSIGNED_INT_24_8;
    ...
    case PIXELFORMAT_DXT1:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT : GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
      break;
    case PIXELFORMAT_DXT3:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT : GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
      break;
    case PIXELFORMAT_DXT5:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT : GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
      break;
    ...
    case PIXELFORMAT_BC7:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM : GL_COMPRESSED_RGBA_BPTC_UNORM;
      break;
    case PIXELFORMAT_PVR1_RGB2:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_PVRTC_2BPPV1_EXT : GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG;
      break;
    case PIXELFORMAT_PVR1_RGB4:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_PVRTC_4BPPV1_EXT : GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG;
      break;
    case PIXELFORMAT_PVR1_RGBA2:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_PVRTC_2BPPV1_EXT : GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG;
      break;
    case PIXELFORMAT_PVR1_RGBA4:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB_ALPHA_PVRTC_4BPPV1_EXT : GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;
      break;
    ...
    case PIXELFORMAT_ETC2_RGBA:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ETC2_EAC : GL_COMPRESSED_RGBA8_ETC2_EAC;
      break;
    case PIXELFORMAT_ETC2_RGBA1:
      f.internalformat = isSRGB ? GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2
    ...
    case PIXELFORMAT_ASTC_4x4:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR : GL_COMPRESSED_RGBA_ASTC_4x4_KHR;
      break;
    case PIXELFORMAT_ASTC_5x4:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4_KHR : GL_COMPRESSED_RGBA_ASTC_5x4_KHR;
      break;
    case PIXELFORMAT_ASTC_5x5:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x5_KHR : GL_COMPRESSED_RGBA_ASTC_5x5_KHR;
      break;
    case PIXELFORMAT_ASTC_6x5:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x5_KHR : GL_COMPRESSED_RGBA_ASTC_6x5_KHR;
      break;
    case PIXELFORMAT_ASTC_6x6:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x6_KHR : GL_COMPRESSED_RGBA_ASTC_6x6_KHR;
      break;
    case PIXELFORMAT_ASTC_8x5:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x5_KHR : GL_COMPRESSED_RGBA_ASTC_8x5_KHR;
      break;
    case PIXELFORMAT_ASTC_8x6:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x6_KHR : GL_COMPRESSED_RGBA_ASTC_8x6_KHR;
      break;
    case PIXELFORMAT_ASTC_8x8:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x8_KHR : GL_COMPRESSED_RGBA_ASTC_8x8_KHR;
      break;
    case PIXELFORMAT_ASTC_10x5:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x5_KHR : GL_COMPRESSED_RGBA_ASTC_10x5_KHR;
      break;
    case PIXELFORMAT_ASTC_10x6:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x6_KHR : GL_COMPRESSED_RGBA_ASTC_10x6_KHR;
      break;
    case PIXELFORMAT_ASTC_10x8:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x8_KHR : GL_COMPRESSED_RGBA_ASTC_10x8_KHR;
      break;
    case PIXELFORMAT_ASTC_10x10:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x10_KHR : GL_COMPRESSED_RGBA_ASTC_10x10_KHR;
      break;
    case PIXELFORMAT_ASTC_12x10:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x10_KHR : GL_COMPRESSED_RGBA_ASTC_12x10_KHR;
      break;
    case PIXELFORMAT_ASTC_12x12:
      f.internalformat =
          isSRGB ? GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x12_KHR : GL_COMPRESSED_RGBA_ASTC_12x12_KHR;
      break;
  ...
  if (!isPixelFormatCompressed(pixelformat))
  {
    ...
    if (GLAD_ES_VERSION_2_0 && !GLAD_ES_VERSION_3_0 && !renderbuffer && !isTexStorageSupported())
    {
...
bool OpenGL::isPixelFormatSupported(PixelFormat pixelformat, bool rendertarget, bool readable,
                                    bool isSRGB)
{
  ...
  switch (pixelformat)
  {
    ...
    case PIXELFORMAT_RG8:
      if (GLAD_VERSION_3_0 || GLAD_ES_VERSION_3_0 || GLAD_ARB_texture_rg || GLAD_EXT_texture_rg)
	return true;
      else if (pixelformat == PIXELFORMAT_R8 && !rendertarget &&
               (GLAD_ES_VERSION_2_0 || GLAD_VERSION_1_1))
    ...
    case PIXELFORMAT_R16:
    case PIXELFORMAT_RG16:
      return GLAD_VERSION_3_0 || (GLAD_VERSION_1_1 && GLAD_ARB_texture_rg) ||
	...
	return GLAD_VERSION_3_0 ||
	       (GLAD_ARB_texture_float && GLAD_ARB_half_float_pixel && GLAD_ARB_texture_rg);
	...
	return GLAD_VERSION_2_0 || GLAD_ES_VERSION_3_0 ||
	       (GLAD_OES_depth_texture && (GLAD_OES_depth24 || GLAD_OES_depth_texture));
      else
	return GLAD_VERSION_2_0 || GLAD_ES_VERSION_3_0 || GLAD_OES_depth24 ||
	       GLAD_OES_depth_texture;

    case PIXELFORMAT_DEPTH24_STENCIL8:
      if (!rendertarget)
	return false;
      else if (readable)
	return GLAD_VERSION_3_0 || GLAD_ES_VERSION_3_0 || GLAD_EXT_packed_depth_stencil ||
	       (GLAD_OES_depth_texture && GLAD_OES_packed_depth_stencil);
      else
	return GLAD_VERSION_3_0 || GLAD_ES_VERSION_3_0 || GLAD_EXT_packed_depth_stencil ||
	       GLAD_OES_packed_depth_stencil;
    ...
    case PIXELFORMAT_DEPTH32F_STENCIL8:
      return rendertarget &&
             (GLAD_VERSION_3_0 || GLAD_ES_VERSION_3_0 || GLAD_ARB_depth_buffer_float);
    ...
    case PIXELFORMAT_BC5:
    case PIXELFORMAT_BC5s:
      return (GLAD_VERSION_3_0 || GLAD_ARB_texture_compression_rgtc ||
              GLAD_EXT_texture_compression_rgtc);
    case PIXELFORMAT_BC6H:
    case PIXELFORMAT_BC6Hs:
    case PIXELFORMAT_BC7:
    ...
    case PIXELFORMAT_ETC1:
      ...
      return GLAD_ES_VERSION_3_0 || GLAD_VERSION_4_3 || GLAD_ARB_ES3_compatibility ||
             GLAD_OES_compressed_ETC1_RGB8_texture;
    case PIXELFORMAT_ETC2_RGB:
    case PIXELFORMAT_ETC2_RGBA:
    case PIXELFORMAT_ETC2_RGBA1:
    case PIXELFORMAT_EAC_R:
