...
void Shader::mapActiveUniforms()
{
  ...
  for (int uindex = 0; uindex < numuniforms; uindex++)
  {
    ...
    if (u.baseType == UNIFORM_SAMPLER && builtin != BUILTIN_TEXTURE_MAIN)
    {
      ...
      for (int i = 0; i < u.count; i++) textureUnits.push_back(unit);
    }
	...
	if (u.baseType == UNIFORM_SAMPLER)
	{
	  ...
	  for (int i = 0; i < u.count; i++) u.ints[i] = startunit + i;
...
void Shader::unloadVolatile()
{
  ...
  for (int i = 0; i < int(BUILTIN_MAX_ENUM); i++) builtinUniforms[i] = -1;
}
...
void Shader::attach()
{
  if (current != this)
  {
    ...
    for (const auto &p : pendingUniformUpdates) updateUniform(p.first, p.second, true);
  ...
  for (int i = 0; i < count; i++)
  {
	...
	if (internalUpdate)
	  ...
	  throw love::Exception(
	      "Textures with non-readable formats cannot be sampled from in a shader.");
      }
	...
	if (internalUpdate)
	  ...
	  throw love::Exception(
	      "Depth comparison samplers in shaders can only be used with depth textures which "
	      "have depth comparison set.");
	else
	  throw love::Exception(
	...
	else
	{
	  ...
	  throw love::Exception("Texture's type (%s) must match the type of %s (%s).", textypestr,
	                        info->name.c_str(), shadertextypestr);
	}
      }
...
ptrdiff_t Shader::getHandle() const { return program; }
...
bool Shader::isSupported() { return GLAD_ES_VERSION_2_0 || (getGLSLVersion() >= "1.2"); }
