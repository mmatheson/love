      ...
      mainVAO(0)
{
  ...
  if (window != nullptr)
  {
    ...
    if (window->isOpen())
    {
      ...
      setMode((int) dpiW, (int) dpiH, window->getPixelWidth(), window->getPixelHeight(),
              settings.stencil);
    }
  }
}
...
const char *Graphics::getName() const { return "love.graphics.opengl"; }
...
love::graphics::Image *Graphics::newImage(const Image::Slices &data,
                                          const Image::Settings &settings)
{
  return new Image(data, settings);
}

love::graphics::Image *Graphics::newImage(TextureType textype, PixelFormat format, int width,
                                          int height, int slices, const Image::Settings &settings)
{
...
love::graphics::ShaderStage *Graphics::newShaderStageInternal(ShaderStage::StageType stage,
                                                              const std::string &cachekey,
                                                              const std::string &source, bool gles)
{
  return new ShaderStage(this, stage, source, gles, cachekey);
}

love::graphics::Shader *Graphics::newShaderInternal(love::graphics::ShaderStage *vertex,
                                                    love::graphics::ShaderStage *pixel)
{
  return new Shader(vertex, pixel);
}

love::graphics::Buffer *Graphics::newBuffer(size_t size, const void *data, BufferType type,
                                            vertex::Usage usage, uint32 mapflags)
{
                       ...
                       bool windowhasstencil)
{
  ...
  if (streamBufferState.vb[0] == nullptr)
  {
    ...
    streamBufferState.indexBuffer =
        CreateStreamBuffer(BUFFER_INDEX, sizeof(uint16) * LOVE_UINT16_MAX);
  }
  ...
  for (int i = 0; i < Shader::STANDARD_MAX_ENUM; i++)
  {
	...
	Shader::standardShaders[i] = love::graphics::Graphics::newShader(
	    code.source[ShaderStage::STAGE_VERTEX], code.source[ShaderStage::STAGE_PIXEL]);
      }
    }
...
void Graphics::unSetMode()
{
  ...
  for (const auto &pair : framebufferObjects) gl.deleteFramebuffer(pair.second);

  for (auto temp : temporaryCanvases) temp.canvas->release();
...
void Graphics::draw(const DrawIndexedCommand &cmd)
{
  ...
  if (cmd.instanceCount > 1)
    glDrawElementsInstanced(glprimitivetype, cmd.indexCount, gldatatype, gloffset,
                            cmd.instanceCount);
  else
...
static inline void advanceVertexOffsets(const vertex::Attributes &attributes,
                                        vertex::BufferBindings &buffers, int vertexcount)
{
...
void Graphics::drawQuads(int start, int count, const vertex::Attributes &attributes,
                         const vertex::BufferBindings &buffers, love::graphics::Texture *texture)
{
  ...
  if (gl.isBaseVertexSupported())
  {
    ...
    for (int quadindex = 0; quadindex < count; quadindex += MAX_QUADS_PER_DRAW)
    {
      ...
      glDrawElementsBaseVertex(GL_TRIANGLES, quadcount * 6, GL_UNSIGNED_SHORT, BUFFER_OFFSET(0),
                               basevertex);
      ++drawCalls;
...
static void APIENTRY debugCB(GLenum source, GLenum type, GLuint id, GLenum severity,
                             GLsizei /*len*/, const GLchar *msg, const GLvoid * /*usr*/)
{
...
void Graphics::setDebug(bool enable)
{
  ...
  glDebugMessageControl(GL_DEBUG_SOURCE_API, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR, GL_DONT_CARE, 0, 0,
                        GL_FALSE);
  glDebugMessageControl(GL_DEBUG_SOURCE_SHADER_COMPILER, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR,
...
void Graphics::setCanvasInternal(const RenderTargets &rts, int w, int h, int pixelw, int pixelh,
                                 bool hasSRGBcanvas)
{
...
void Graphics::endPass()
{
  ...
  if (depthstencil == nullptr &&
      (rts.temporaryRTFlags & (TEMPORARY_RT_DEPTH | TEMPORARY_RT_STENCIL)) != 0)
    discard({}, true);
...
void Graphics::clear(const std::vector<OptionalColorf> &colors, OptionalInt stencil,
                     OptionalDouble depth)
{
  ...
  if (drawbuffersmodified)
  {
    ...
    for (int i = 0; i < ncolorcanvases; i++) bufs[i] = GL_COLOR_ATTACHMENT0 + i;
...
void Graphics::discard(OpenGL::FramebufferTarget target, const std::vector<bool> &colorbuffers,
                       bool depthstencil)
{
...
void Graphics::bindCachedFBO(const RenderTargets &targets)
{
  ...
  else
  {
    ...
    auto attachCanvas = [&](const RenderTarget &rt)
    {
      ...
      bool srgb = false;
      OpenGL::TextureFormat fmt =
          OpenGL::convertPixelFormat(rt.canvas->getPixelFormat(), renderbuffer, srgb);
    ...
    for (const auto &rt : targets.colors) attachCanvas(rt);
...
void Graphics::present(void *screenshotCallbackData)
{
  ...
  for (StreamBuffer *buffer : streamBufferState.vb) buffer->nextFrame();
...
void Graphics::drawToStencilBuffer(StencilAction action, int value)
{
  ...
  if (!isCanvasActive() && !windowHasStencil)
    throw love::Exception(
        "The window must have stenciling enabled to draw to the main screen's stencil buffer.");
  else if (isCanvasActive() && (rts.temporaryRTFlags & TEMPORARY_RT_STENCIL) == 0 &&
           (dscanvas == nullptr || !isPixelFormatStencil(dscanvas->getPixelFormat())))
    throw love::Exception(
        "Drawing to the stencil buffer with a Canvas active requires either stencil=true or a "
        "custom stencil-type Canvas to be used, in setCanvas.");
...
void Graphics::setBlendMode(BlendMode mode, BlendAlpha alphamode)
{
  ...
  if (mode == BLEND_LIGHTEN || mode == BLEND_DARKEN)
  {
    if (!capabilities.features[FEATURE_LIGHTEN])
      throw love::Exception(
          "The 'lighten' and 'darken' blend modes are not supported on this system.");
  }
...
void Graphics::getAPIStats(int &shaderswitches) const { shaderswitches = gl.stats.shaderSwitches; }
...
void Graphics::initCapabilities()
{
  ...
  capabilities.features[FEATURE_LIGHTEN] =
      GLAD_VERSION_1_4 || GLAD_ES_VERSION_3_0 || GLAD_EXT_blend_minmax;
  capabilities.features[FEATURE_FULL_NPOT] =
      GLAD_VERSION_2_0 || GLAD_ES_VERSION_3_0 || GLAD_OES_texture_npot;
  capabilities.features[FEATURE_PIXEL_SHADER_HIGHP] = gl.isPixelShaderHighpSupported();
  capabilities.features[FEATURE_SHADER_DERIVATIVES] =
      GLAD_VERSION_2_0 || GLAD_ES_VERSION_3_0 || GLAD_OES_standard_derivatives;
  ...
  static_assert(FEATURE_MAX_ENUM == 8,
                "Graphics::initCapabilities must be updated when adding a new graphics feature!");
  ...
  static_assert(LIMIT_MAX_ENUM == 8,
                "Graphics::initCapabilities must be updated when adding a new system limit!");
