...
void Image::loadData()
{
  ...
  if (!isCompressed())
    gl.rawTexStorage(texType, mipcount, format, sRGB, pixelWidth, pixelHeight,
  ...
  for (int mip = 0; mip < mipcount; mip++)
  {
    if (isCompressed() && (texType == TEXTURE_2D_ARRAY || texType == TEXTURE_VOLUME))
    {
      ...
      for (int slice = 0; slice < mipslices; slice++) mipsize += data.get(slice, mip)->getSize();

      GLenum gltarget = OpenGL::getGLTextureType(texType);
      glCompressedTexImage3D(gltarget, mip, fmt.internalformat, w, h, mipslices, 0, mipsize,
                             nullptr);
    }
...
void Image::uploadByteData(PixelFormat pixelformat, const void *data, size_t size, int level,
                           int slice, const Rect &r)
{
  ...
  if (isPixelFormatCompressed(pixelformat))
  {
    ...
    if (texType == TEXTURE_2D || texType == TEXTURE_CUBE)
      ...
      glCompressedTexSubImage3D(gltarget, level, 0, 0, slice, r.w, r.h, 1, fmt.internalformat, size,
                                data);
  }
  else
  {
    if (texType == TEXTURE_2D || texType == TEXTURE_CUBE)
      ...
      glTexSubImage3D(gltarget, level, r.x, r.y, slice, r.w, r.h, 1, fmt.externalformat, fmt.type,
                      data);
  }
}
...
ptrdiff_t Image::getHandle() const { return texture; }
