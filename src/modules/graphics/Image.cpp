...
Image::Image(TextureType textype, PixelFormat format, int width, int height, int slices,
  ...
  if (gfx != nullptr && !gfx->isImageFormatSupported(fmt, sRGB))
  {
    ...
    if (love::getConstant(fmt, str))
    {
      throw love::Exception(
          "Cannot create image: "
          "%s%s images are not supported on this system.",
          sRGB ? "sRGB " : "", str);
    }
    else
...
void Image::replacePixels(love::image::ImageDataBase *d, int slice, int mipmap, int x, int y,
                          bool reloadmipmaps)
{
  ...
  if (rect.x < 0 || rect.y < 0 || rect.w <= 0 || rect.h <= 0 || (rect.x + rect.w) > mipw ||
      (rect.y + rect.h) > miph)
  {
    throw love::Exception("Invalid rectangle dimensions (x=%d, y=%d, w=%d, h=%d) for %dx%d Image.",
                          rect.x, rect.y, rect.w, rect.h, mipw, miph);
  }
...
void Image::replacePixels(const void *data, size_t size, int slice, int mipmap, const Rect &rect,
                          bool reloadmipmaps)
{
  Graphics::flushStreamDrawsGlobal();
...
bool Image::isCompressed() const { return isPixelFormatCompressed(format); }

bool Image::isFormatLinear() const { return isGammaCorrect() && !sRGB; }

Image::MipmapsType Image::getMipmapsType() const { return mipmapsType; }
...
void Image::Slices::clear() { data.clear(); }
...
void Image::Slices::add(love::image::CompressedImageData *cdata, int startslice, int startmip,
                        bool addallslices, bool addallmips)
{
...
Image::MipmapsType Image::Slices::validate() const
{
  ...
  if (mipcount != expectedmips && mipcount != 1)
    throw love::Exception("Image does not have all required mipmap levels (expected %d, got %d)",
  ...
  for (int mip = 0; mip < mipcount; mip++)
  {
	...
	throw love::Exception(
	    "Invalid number of image data layers in mipmap level %d (expected %d, got %d)", mip + 1,
	    mipslices, slicecount);
    }
	...
	throw love::Exception("All Image layers must have the same mipmap count.");
      ...
      if (mipw != realw)
	throw love::Exception(
	    "Width of image data (slice %d, mipmap level %d) is incorrect (expected %d, got %d)",
	    slice + 1, mip + 1, mipw, realw);

      if (miph != realh)
	throw love::Exception(
	    "Height of image data (slice %d, mipmap level %d) is incorrect (expected %d, got %d)",
...
bool Image::getConstant(const char *in, SettingType &out) { return settingTypes.find(in, out); }

bool Image::getConstant(SettingType in, const char *&out) { return settingTypes.find(in, out); }
...
std::vector<std::string> Image::getConstants(SettingType) { return settingTypes.getNames(); }
...
StringMap<Image::SettingType, Image::SETTING_MAX_ENUM> Image::settingTypes(
    Image::settingTypeEntries, sizeof(Image::settingTypeEntries));
