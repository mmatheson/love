...
static_assert(sizeof(STf_RGBAub) == sizeof(float) * 2 + sizeof(Color32),
              "sizeof(STf_RGBAub) incorrect!");
static_assert(sizeof(STPf_RGBAub) == sizeof(float) * 3 + sizeof(Color32),
              "sizeof(STPf_RGBAub) incorrect!");
static_assert(sizeof(XYf_STf) == sizeof(float) * 2 + sizeof(float) * 2,
              "sizeof(XYf_STf) incorrect!");
static_assert(sizeof(XYf_STPf) == sizeof(float) * 2 + sizeof(float) * 3,
              "sizeof(XYf_STPf) incorrect!");
static_assert(sizeof(XYf_STf_RGBAub) == sizeof(float) * 2 + sizeof(float) * 2 + sizeof(Color32),
              "sizeof(XYf_STf_RGBAub) incorrect!");
static_assert(sizeof(XYf_STus_RGBAub) == sizeof(float) * 2 + sizeof(uint16) * 2 + sizeof(Color32),
              "sizeof(XYf_STus_RGBAub) incorrect!");
static_assert(sizeof(XYf_STPf_RGBAub) == sizeof(float) * 2 + sizeof(float) * 3 + sizeof(Color32),
              "sizeof(XYf_STPf_RGBAub) incorrect!");
...
void Attributes::setCommonFormat(CommonFormat format, uint8 bufferindex)
{
  ...
  switch (format)
  {
    ...
    case CommonFormat::XYf_STus_RGBAub:
      ...
      set(ATTRIB_COLOR, DATA_UNORM8, 4, uint16(sizeof(float) * 2 + sizeof(uint16) * 2),
          bufferindex);
      break;
    case CommonFormat::XYf_STPf_RGBAub:
      set(ATTRIB_POS, DATA_FLOAT, 2, 0, bufferindex);
...
static StringMap<BuiltinVertexAttribute, ATTRIB_MAX_ENUM> attribNames(attribNameEntries,
                                                                      sizeof(attribNameEntries));
...
static StringMap<IndexDataType, INDEX_MAX_ENUM> indexTypes(indexTypeEntries,
                                                           sizeof(indexTypeEntries));
...
static StringMap<PrimitiveType, PRIMITIVE_MAX_ENUM> primitiveTypes(primitiveTypeEntries,
                                                                   sizeof(primitiveTypeEntries));
...
static StringMap<AttributeStep, STEP_MAX_ENUM> attributeSteps(attributeStepEntries,
                                                              sizeof(attributeStepEntries));
...
bool getConstant(const char *in, BuiltinVertexAttribute &out) { return attribNames.find(in, out); }

bool getConstant(BuiltinVertexAttribute in, const char *&out) { return attribNames.find(in, out); }

bool getConstant(const char *in, IndexDataType &out) { return indexTypes.find(in, out); }

bool getConstant(IndexDataType in, const char *&out) { return indexTypes.find(in, out); }

std::vector<std::string> getConstants(IndexDataType) { return indexTypes.getNames(); }

bool getConstant(const char *in, Usage &out) { return usages.find(in, out); }

bool getConstant(Usage in, const char *&out) { return usages.find(in, out); }

std::vector<std::string> getConstants(Usage) { return usages.getNames(); }

bool getConstant(const char *in, PrimitiveType &out) { return primitiveTypes.find(in, out); }

bool getConstant(PrimitiveType in, const char *&out) { return primitiveTypes.find(in, out); }

std::vector<std::string> getConstants(PrimitiveType) { return primitiveTypes.getNames(); }

bool getConstant(const char *in, AttributeStep &out) { return attributeSteps.find(in, out); }

bool getConstant(AttributeStep in, const char *&out) { return attributeSteps.find(in, out); }

std::vector<std::string> getConstants(AttributeStep) { return attributeSteps.getNames(); }

bool getConstant(const char *in, DataType &out) { return dataTypes.find(in, out); }

bool getConstant(DataType in, const char *&out) { return dataTypes.find(in, out); }

std::vector<std::string> getConstants(DataType) { return dataTypes.getNames(); }

bool getConstant(const char *in, CullMode &out) { return cullModes.find(in, out); }

bool getConstant(CullMode in, const char *&out) { return cullModes.find(in, out); }

std::vector<std::string> getConstants(CullMode) { return cullModes.getNames(); }

bool getConstant(const char *in, Winding &out) { return windings.find(in, out); }

bool getConstant(Winding in, const char *&out) { return windings.find(in, out); }

std::vector<std::string> getConstants(Winding) { return windings.getNames(); }
