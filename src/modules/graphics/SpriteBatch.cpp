      ...
      range_count(-1)
{
  ...
  array_buf =
      gfx->newBuffer(vertex_size, nullptr, BUFFER_VERTEX, usage, Buffer::MAP_EXPLICIT_RANGE_MODIFY);
}

SpriteBatch::~SpriteBatch() { delete array_buf; }
...
int SpriteBatch::addLayer(int layer, Quad *quad, const Matrix4 &m, int index)
{
  ...
  if (vertex_format != CommonFormat::XYf_STPf_RGBAub)
    throw love::Exception(
        "addLayer can only be called on a SpriteBatch that uses an Array Texture!");
...
void SpriteBatch::flush() { array_buf->unmap(); }
...
Texture *SpriteBatch::getTexture() const { return texture.get(); }
...
int SpriteBatch::getCount() const { return next; }
...
void SpriteBatch::setBufferSize(int newsize)
{
  ...
  try
  {
    ...
    new_array_buf = gfx->newBuffer(vertex_size, nullptr, array_buf->getType(),
                                   array_buf->getUsage(), array_buf->getMapFlags());
...
int SpriteBatch::getBufferSize() const { return size; }
...
void SpriteBatch::attachAttribute(const std::string &name, Mesh *mesh)
{
  ...
  if (mesh->getVertexCount() < (size_t) next * 4)
    throw love::Exception(
        "Mesh has too few vertices to be attached to this SpriteBatch (at least %d vertices are "
        "required)",
        next * 4);
...
void SpriteBatch::setDrawRange() { range_start = range_count = -1; }
...
void SpriteBatch::draw(Graphics *gfx, const Matrix4 &m)
{
  ...
  for (const auto &it : attached_attributes)
  {
    ...
    if (mesh->getVertexCount() < (size_t) next * 4)
      throw love::Exception(
          "Mesh with attribute '%s' attached to this SpriteBatch has too few vertices",
          it.first.c_str());
