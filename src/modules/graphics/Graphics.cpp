...
void setGammaCorrect(bool gammacorrect) { gammaCorrect = gammacorrect; }

bool isGammaCorrect() { return gammaCorrect; }
...
Graphics::DefaultShaderCode Graphics::defaultShaderCode[Shader::STANDARD_MAX_ENUM]
                                                       [Shader::LANGUAGE_MAX_ENUM][2];
...
Graphics::~Graphics()
{
  ...
  for (int i = 0; i < (int) ShaderStage::STAGE_MAX_ENUM; i++) cachedShaderStages[i].clear();
...
Quad *Graphics::newQuad(Quad::Viewport v, double sw, double sh) { return new Quad(v, sw, sh); }
                               ...
                               const Texture::Filter &filter)
{
  ...
  StrongRef<font::Rasterizer> r(fontmodule->newTrueTypeRasterizer(size, hinting),
                                Acquire::NORETAIN);
...
love::graphics::SpriteBatch *Graphics::newSpriteBatch(Texture *texture, int size,
                                                      vertex::Usage usage)
{
...
ShaderStage *Graphics::newShaderStage(ShaderStage::StageType stage, const std::string &optsource)
{
  ...
  const std::string &source =
      optsource.empty() ? getCurrentDefaultShaderCode().source[stage] : optsource;
...
Shader *Graphics::newShader(const std::string &vertex, const std::string &pixel)
{
  ...
  StrongRef<ShaderStage> vertexstage(newShaderStage(ShaderStage::STAGE_VERTEX, vertex),
                                     Acquire::NORETAIN);
  StrongRef<ShaderStage> pixelstage(newShaderStage(ShaderStage::STAGE_PIXEL, pixel),
                                    Acquire::NORETAIN);

  return newShaderInternal(vertexstage.get(), pixelstage.get());
}

Mesh *Graphics::newMesh(const std::vector<Vertex> &vertices, PrimitiveType drawmode,
                        vertex::Usage usage)
{
  return newMesh(Mesh::getDefaultVertexFormat(), &vertices[0], vertices.size() * sizeof(Vertex),
                 drawmode, usage);
}
...
love::graphics::Mesh *Graphics::newMesh(const std::vector<Mesh::AttribFormat> &vertexformat,
                                        int vertexcount, PrimitiveType drawmode,
                                        vertex::Usage usage)
{
...
love::graphics::Text *Graphics::newText(graphics::Font *font,
                                        const std::vector<Font::ColoredString> &text)
{
...
bool Graphics::validateShader(bool gles, const std::string &vertex, const std::string &pixel,
                              std::string &err)
{
  if (vertex.empty() && pixel.empty())
  {
  ...
  // current renderer.
  if (!vertex.empty())
    vertexstage.set(new ShaderStageForValidation(this, ShaderStage::STAGE_VERTEX, vertex, gles),
                    Acquire::NORETAIN);

  if (!pixel.empty())
    pixelstage.set(new ShaderStageForValidation(this, ShaderStage::STAGE_PIXEL, pixel, gles),
                   Acquire::NORETAIN);
...
int Graphics::getPixelWidth() const { return pixelWidth; }

int Graphics::getPixelHeight() const { return pixelHeight; }
...
bool Graphics::isCreated() const { return created; }
...
Colorf Graphics::getColor() const { return states.back().color; }

void Graphics::setBackgroundColor(Colorf c) { states.back().backgroundColor = c; }

Colorf Graphics::getBackgroundColor() const { return states.back().backgroundColor; }
...
void Graphics::checkSetDefaultFont()
{
  ...
  if (!defaultFont.get())
    defaultFont.set(newDefaultFont(12, font::TrueTypeRasterizer::HINTING_NORMAL),
                    Acquire::NORETAIN);
...
love::graphics::Shader *Graphics::getShader() const { return states.back().shader.get(); }
...
void Graphics::setCanvas(const RenderTargetsStrongRef &rts)
{
  ...
  targets.depthStencil =
      RenderTarget(rts.depthStencil.canvas, rts.depthStencil.slice, rts.depthStencil.mipmap);
...
void Graphics::setCanvas(const RenderTargets &rts)
{
  ...
  if (isPixelFormatDepthStencil(firstcolorformat))
    throw love::Exception(
        "Depth/stencil format Canvases must be used with the 'depthstencil' field of the table "
        "passed into setCanvas.");
  ...
  for (int i = 1; i < ncanvases; i++)
  {
    ...
    if (!multiformatsupported && format != firstcolorformat)
      throw love::Exception(
          "This system doesn't support multi-canvas rendering with different canvas formats.");
  ...
  if (rts.depthStencil.canvas != nullptr)
  {
    ...
    if (!isPixelFormatDepthStencil(c->getPixelFormat()))
      throw love::Exception(
          "Only depth/stencil format Canvases can be used with the 'depthstencil' field of the "
          "table passed into setCanvas.");
  ...
  for (auto c : rts.colors) refs.colors.emplace_back(c.canvas, c.slice, c.mipmap);
...
Canvas *Graphics::getTemporaryCanvas(PixelFormat format, int w, int h, int samples)
{
  ...
  for (TemporaryCanvas &temp : temporaryCanvases)
  {
    ...
    if (c->getPixelFormat() == format && c->getPixelWidth() == w && c->getPixelHeight() == h &&
        c->getRequestedMSAA() == samples)
    {
      canvas = c;
...
void Graphics::setStencilTest() { setStencilTest(COMPARE_ALWAYS, 0); }
...
void Graphics::setDepthMode() { setDepthMode(COMPARE_ALWAYS, false); }
...
CullMode Graphics::getMeshCullMode() const { return states.back().meshCullMode; }

vertex::Winding Graphics::getFrontFaceWinding() const { return states.back().winding; }

Graphics::ColorMask Graphics::getColorMask() const { return states.back().colorMask; }
...
const Texture::Filter &Graphics::getDefaultFilter() const { return Texture::defaultFilter; }
...
void Graphics::setLineWidth(float width) { states.back().lineWidth = width; }

void Graphics::setLineStyle(Graphics::LineStyle style) { states.back().lineStyle = style; }

void Graphics::setLineJoin(Graphics::LineJoin join) { states.back().lineJoin = join; }

float Graphics::getLineWidth() const { return states.back().lineWidth; }

Graphics::LineStyle Graphics::getLineStyle() const { return states.back().lineStyle; }

Graphics::LineJoin Graphics::getLineJoin() const { return states.back().lineJoin; }

float Graphics::getPointSize() const { return states.back().pointSize; }

bool Graphics::isWireframe() const { return states.back().wireframe; }
...
Graphics::StreamVertexData Graphics::requestStreamDraw(const StreamDrawCommand &cmd)
{
  ...
  if (cmd.primitiveMode != state.primitiveMode || cmd.formats[0] != state.formats[0] ||
      ...
      cmd.texture != state.texture || cmd.standardShaderType != state.standardShaderType)
  {
    shouldflush = true;
  }
...
void Graphics::draw(Drawable *drawable, const Matrix4 &m) { drawable->draw(this, m); }
...
void Graphics::printf(const std::vector<Font::ColoredString> &str, float wrap,
                      Font::AlignMode align, const Matrix4 &m)
{
  checkSetDefaultFont();
...
void Graphics::printf(const std::vector<Font::ColoredString> &str, Font *font, float wrap,
                      Font::AlignMode align, const Matrix4 &m)
{
...
void Graphics::points(const Vector2 *positions, const Colorf *colors, size_t numpoints)
{
  ...
  if (colors)
  {
    ...
    {
      for (int i = 0; i < cmd.vertexCount; i++) colordata[i] = toColor32(nc * colors[i]);
    }
  }
  else
  {
    ...
    for (int i = 0; i < cmd.vertexCount; i++) colordata[i] = c;
  }
}
...
void Graphics::arc(DrawMode drawmode, ArcMode arcmode, float x, float y, float radius, float angle1,
...
const Graphics::Capabilities &Graphics::getCapabilities() const { return capabilities; }
...
size_t Graphics::getStackDepth() const { return stackTypeStack.size(); }
...
const Matrix4 &Graphics::getTransform() const { return transformStack.back(); }

const Matrix4 &Graphics::getProjection() const { return projectionMatrix; }

void Graphics::pushTransform() { transformStack.push_back(transformStack.back()); }

void Graphics::pushIdentityTransform() { transformStack.push_back(Matrix4()); }

void Graphics::popTransform() { transformStack.pop_back(); }

void Graphics::rotate(float r) { transformStack.back().rotate(r); }
...
void Graphics::translate(float x, float y) { transformStack.back().translate(x, y); }

void Graphics::shear(float kx, float ky) { transformStack.back().shear(kx, ky); }
...
bool Graphics::getConstant(const char *in, DrawMode &out) { return drawModes.find(in, out); }

bool Graphics::getConstant(DrawMode in, const char *&out) { return drawModes.find(in, out); }

std::vector<std::string> Graphics::getConstants(DrawMode) { return drawModes.getNames(); }

bool Graphics::getConstant(const char *in, ArcMode &out) { return arcModes.find(in, out); }

bool Graphics::getConstant(ArcMode in, const char *&out) { return arcModes.find(in, out); }

std::vector<std::string> Graphics::getConstants(ArcMode) { return arcModes.getNames(); }

bool Graphics::getConstant(const char *in, BlendMode &out) { return blendModes.find(in, out); }

bool Graphics::getConstant(BlendMode in, const char *&out) { return blendModes.find(in, out); }

std::vector<std::string> Graphics::getConstants(BlendMode) { return blendModes.getNames(); }
...
std::vector<std::string> Graphics::getConstants(BlendAlpha) { return blendAlphaModes.getNames(); }

bool Graphics::getConstant(const char *in, LineStyle &out) { return lineStyles.find(in, out); }

bool Graphics::getConstant(LineStyle in, const char *&out) { return lineStyles.find(in, out); }

std::vector<std::string> Graphics::getConstants(LineStyle) { return lineStyles.getNames(); }

bool Graphics::getConstant(const char *in, LineJoin &out) { return lineJoins.find(in, out); }

bool Graphics::getConstant(LineJoin in, const char *&out) { return lineJoins.find(in, out); }

std::vector<std::string> Graphics::getConstants(LineJoin) { return lineJoins.getNames(); }

bool Graphics::getConstant(const char *in, Feature &out) { return features.find(in, out); }

bool Graphics::getConstant(Feature in, const char *&out) { return features.find(in, out); }

bool Graphics::getConstant(const char *in, SystemLimit &out) { return systemLimits.find(in, out); }

bool Graphics::getConstant(SystemLimit in, const char *&out) { return systemLimits.find(in, out); }

bool Graphics::getConstant(const char *in, StackType &out) { return stackTypes.find(in, out); }

bool Graphics::getConstant(StackType in, const char *&out) { return stackTypes.find(in, out); }

std::vector<std::string> Graphics::getConstants(StackType) { return stackTypes.getNames(); }
...
StringMap<Graphics::LineStyle, Graphics::LINE_MAX_ENUM>::Entry Graphics::lineStyleEntries[] = {
    {"smooth", LINE_SMOOTH}, {"rough", LINE_ROUGH}};

StringMap<Graphics::LineStyle, Graphics::LINE_MAX_ENUM> Graphics::lineStyles(
    Graphics::lineStyleEntries, sizeof(Graphics::lineStyleEntries));

StringMap<Graphics::LineJoin, Graphics::LINE_JOIN_MAX_ENUM>::Entry Graphics::lineJoinEntries[] = {
    {"none", LINE_JOIN_NONE}, {"miter", LINE_JOIN_MITER}, {"bevel", LINE_JOIN_BEVEL}};

StringMap<Graphics::LineJoin, Graphics::LINE_JOIN_MAX_ENUM> Graphics::lineJoins(
    Graphics::lineJoinEntries, sizeof(Graphics::lineJoinEntries));
...
StringMap<Graphics::Feature, Graphics::FEATURE_MAX_ENUM> Graphics::features(
    Graphics::featureEntries, sizeof(Graphics::featureEntries));
...
StringMap<Graphics::SystemLimit, Graphics::LIMIT_MAX_ENUM> Graphics::systemLimits(
    Graphics::systemLimitEntries, sizeof(Graphics::systemLimitEntries));
