...
ByteData *DataModule::newByteData(size_t size) { return new ByteData(size); }

ByteData *DataModule::newByteData(const void *d, size_t size) { return new ByteData(d, size); }
...
static StringMap<ContainerType, CONTAINER_MAX_ENUM> containers(containerEntries,
                                                               sizeof(containerEntries));

bool getConstant(const char *in, EncodeFormat &out) { return encoders.find(in, out); }

bool getConstant(EncodeFormat in, const char *&out) { return encoders.find(in, out); }

std::vector<std::string> getConstants(EncodeFormat) { return encoders.getNames(); }

bool getConstant(const char *in, ContainerType &out) { return containers.find(in, out); }

bool getConstant(ContainerType in, const char *&out) { return containers.find(in, out); }

std::vector<std::string> getConstants(ContainerType) { return containers.getNames(); }
