...
// FIXME: Probably trivial by having tole and tobe functions, which can be ifdeffed to being
// identity functions
...
inline uint32 leftrot(uint32 x, uint8 amount) { return (x << amount) | (x >> (32 - amount)); }

inline uint32 rightrot(uint32 x, uint8 amount) { return (x >> amount) | (x << (32 - amount)); }

inline uint64 rightrot(uint64 x, uint8 amount) { return (x >> amount) | (x << (64 - amount)); }
...
class MD5 : public HashFunction
{
 ...
 public:
  bool isSupported(Function function) const override { return function == FUNCTION_MD5; }
...
{
 public:
  bool isSupported(Function function) const override { return function == FUNCTION_SHA1; }
  ...
  void hash(Function function, const char *input, uint64 length, Value &output) const override
  {
    ...
    uint32 intermediate[5] = {0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0};
...
std::vector<std::string> HashFunction::getConstants(Function) { return functionNames.getNames(); }
