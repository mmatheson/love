...
Data *luax_checkdata(lua_State *L, int idx) { return luax_checktype<Data>(L, idx); }
...
static FFI_Data ffifuncs = {[](Proxy *p) -> void *  // getFFIPointer
                            {
                              auto data = luax_ffi_checktype<Data>(p);
                              return data != nullptr ? data->getData() : nullptr;
                            }};

const luaL_Reg w_Data_functions[] = {{"getString", w_Data_getString},
