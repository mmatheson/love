...
{
 public:
  char *compress(Format format, const char *data, size_t dataSize, int level,
                 size_t &compressedSize) override
  {
    ...
    *(uint32 *) compressedbytes = (uint32) dataSize;
    ...
    int csize = 0;
    if (level > 8)
      csize = LZ4_compress_HC(data, compressedbytes + headersize, (int) dataSize, maxdestsize,
  ...
  char *decompress(Format format, const char *data, size_t dataSize,
                   size_t &decompressedSize) override
  {
    ...
    else
    {
      ...
      int result =
          LZ4_decompress_safe(data + headersize, rawbytes, (int) (dataSize - headersize), rawsize);
  ...
  bool isSupported(Format format) const override { return format == FORMAT_LZ4; }
...
class zlibCompressor : public Compressor
{
 private:
  ...
  int zlibCompress(Format format, Bytef *dest, uLongf *destLen, const Bytef *source,
  ...
  int zlibDecompress(Format format, Bytef *dest, uLongf *destLen, const Bytef *source,
                     uLong sourceLen)
  {
    z_stream stream = {};
 ...
 public:
                 ...
                 size_t &compressedSize) override
  {
    ...
    int status = zlibCompress(format, (Bytef *) compressedbytes, &destlen, (const Bytef *) data,
                   ...
                   size_t &decompressedSize) override
  {
	...
	throw love::Exception("Out of memory.");
      ...
      int status = zlibDecompress(format, (Bytef *) rawbytes, &destLen, (const Bytef *) data,
                                  (uLong) dataSize);
...
bool Compressor::getConstant(const char *in, Format &out) { return formatNames.find(in, out); }

bool Compressor::getConstant(Format in, const char *&out) { return formatNames.find(in, out); }

std::vector<std::string> Compressor::getConstants(Format) { return formatNames.getNames(); }
