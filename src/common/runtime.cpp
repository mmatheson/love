...
bool luax_toboolean(lua_State *L, int idx) { return (lua_toboolean(L, idx) != 0); }
...
void luax_pushboolean(lua_State *L, bool b) { lua_pushboolean(L, b ? 1 : 0); }
...
int luax_assert_argc(lua_State *L, int min)
{
  ...
  if (argc < min)
    return luaL_error(L, "Incorrect number of arguments. Got [%d], expected at least [%d]", argc,
                      min);
  return 0;
}
...
int luax_assert_argc(lua_State *L, int min, int max)
{
  ...
  if (argc < min || argc > max)
    return luaL_error(L, "Incorrect number of arguments. Got [%d], expected [%d-%d]", argc, min,
                      max);
  return 0;
}
...
int luax_register_module(lua_State *L, const WrappedModule &m)
{
  ...
  if (m.types != nullptr)
  {
    for (const lua_CFunction *t = m.types; *t != nullptr; t++) (*t)(L);
  }
...
int luax_c_insistglobal(lua_State *L, const char *k) { return luax_insistglobal(L, k); }
...
Type *luax_type(lua_State *L, int idx) { return Type::byName(luaL_checkstring(L, idx)); }
